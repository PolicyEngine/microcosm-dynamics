"""Validation and mutation coverage for the sealed fam2005_QxQs shard."""
...
import build_rq_stage2_document_066 as builder  # noqa: E402
...
def _capture_root() -> Path:
    ...
    if not (capture_root / builder.PDF_FILENAME).exists():
        pytest.skip("staged PSID fam2005_QxQs.pdf is unavailable")
    return capture_root
...
def test_document_066_reproduces_committed_sealed_annotation():
...
def test_document_066_sealed_census_is_pinned():
    """Pin the reviewer's retained census so a silent drift fails closed."""

    committed = json.loads(builder.OUTPUT_PATH.read_text(encoding="utf-8"))
    occurrences = committed["questionnaire_occurrence_rows"]
    assert len(committed["whole_document_locator_rows"]) == 1
    assert len(committed["questionnaire_page_rows"]) == builder.PAGE_COUNT
    empty_pages = [
        row["page_number"]
        for row in committed["questionnaire_page_rows"]
        if not row["questionnaire_occurrence_ids"]
    ]
    assert len(empty_pages) == 73
    assert len(occurrences) == 269
    assert collections.Counter(
        row["occurrence_kind"] for row in occurrences
    ) == {
        "flow_branch_label": 18,
        "role_anchor": 13,
        "job_anchor": 24,
        "remuneration_component_anchor": 24,
        "role_total_anchor": 4,
        "farm_aggregate_anchor": 4,
        "business_aggregate_anchor": 9,
        "context_anchor": 61,
        "field_purpose_prompt": 85,
        "repeat_or_alias_instruction": 27,
    }
    assert len(committed["flow_branch_rows"]) == 18
    assert len(committed["local_anchor_classification_rows"]) == 139
    assert len(committed["local_field_purpose_classification_rows"]) == 85
    repeats = committed["local_repeat_or_alias_evidence_rows"]
    assert len(repeats) == 29
    assert (
        sum(1 for row in repeats if row["unresolved_target_reference"]) == 17
    )
    assert (
        len(committed["candidate_disposition_rows"])
        == builder.CANDIDATE_DENOMINATOR
        == 3948
    )
    assert collections.Counter(
        (row["candidate_row_kind"], row["disposition"])
        for row in committed["candidate_disposition_rows"]
    ) == {
        ("whole_document_locator", "accepted"): 1,
        ("page", "accepted"): 97,
        ("occurrence", "accepted"): 75,
        ("occurrence", "modified"): 95,
        ("occurrence", "rejected"): 1741,
        ("flow_path", "accepted"): 7,
        ("flow_path", "rejected"): 923,
        ("anchor_classification", "accepted"): 5,
        ("anchor_classification", "modified"): 98,
        ("anchor_classification", "rejected"): 906,
    }
    assert collections.Counter(
        row["adjudication_action"]
        for row in committed["output_adjudication_rows"]
    ) == {
        "candidate_accepted": 180,
        "candidate_modified": 152,
        "manual_add": 306,
    }
    assert len(committed["correction_note_rows"]) == 4069


def test_document_066_committed_shard_verifies_against_source_bytes():
...
def test_document_066_mutations_fail_closed():
    capture_root = _capture_root()
    ...
    # This shard has no multi-parent branch label, so a selected path subset
    # shows up as a conditional occurrence demoted to the root path.
