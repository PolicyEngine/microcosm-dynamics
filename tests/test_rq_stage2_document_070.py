"""Validation and mutation coverage for the sealed fam2009_QxQs shard."""
...
import build_rq_stage2_document_070 as builder  # noqa: E402
...
def _capture_root() -> Path:
    ...
    if not (capture_root / builder.PDF_FILENAME).exists():
        pytest.skip("staged PSID fam2009_QxQs.pdf is unavailable")
    return capture_root
...
def test_document_070_reproduces_committed_sealed_annotation():
...
def test_document_070_committed_shard_verifies_against_source_bytes():
...
def test_document_070_mutations_fail_closed():
    capture_root = _capture_root()
    ...
    # Every fam2009_QxQs occurrence resolves under exactly one branch path,
