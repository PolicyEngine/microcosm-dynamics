"""Independent validation of the sealed R_Q document-10 annotation.

Every row assertion below is re-derived from ``q72.pdf`` through the
...
SOURCE_PDF = CAPTURE_ROOT / "q72.pdf"
ANNOTATION_PATH = (
    ROOT
    / "docs"
    / "analysis"
    / "rq_stage2_annotations"
    / "document_010_q72_annotation_v1.json"
)
...
FILE_SIZE = 1_751_436
FILE_SHA256 = (
    "0d272752345be9bc06e12afb76478476b53c9c77c85bbdcd9f29655deafc1c3d"
)
...
def test_whole_document_locator_obeys_every_equation(sealed: dict) -> None:
    ...
    assert locator["filename"] == "q72.pdf"
...
def test_empty_occurrence_pages_are_still_emitted(sealed: dict) -> None:
    ...
    assert len(empty) == PAGE_COUNT - 10
    assert {
...
def test_atomic_span_kind_uniqueness_and_zero_ordinals(sealed: dict) -> None:
    seen: dict[tuple[int, int, int, str], list[dict]] = {}
    for row in sealed["questionnaire_occurrence_rows"]:
        coordinate = (
            row["page_number"],
            row["utf8_byte_start"],
        ...
        seen.setdefault(coordinate, []).append(row)
