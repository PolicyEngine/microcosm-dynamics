"""Independent validation of the sealed R_Q document-20 annotation.

Every row assertion below is re-derived from ``q77.pdf`` through the
...
SOURCE_PDF = CAPTURE_ROOT / "q77.pdf"
ANNOTATION_PATH = (
    ROOT
    / "docs"
    / "analysis"
    / "rq_stage2_annotations"
    / "document_020_q77_annotation_v1.json"
)
...
FILE_SIZE = 1_390_341
FILE_SHA256 = (
    "aa1c46c0bf08e44d3f8cd85e4fcbf637f634a6f00992d30bda8abe4a54debc4c"
)
...
def test_whole_document_locator_obeys_every_equation(sealed: dict) -> None:
    ...
    assert locator["filename"] == "q77.pdf"
...
def test_atomic_span_kind_uniqueness_and_lawful_ordinals(
    sealed: dict,
) -> None:
    rows_by_coordinate: dict[tuple[int, int, int, str], list[dict]] = {}
    for row in sealed["questionnaire_occurrence_rows"]:
        coordinate = (
            row["page_number"],
            row["utf8_byte_start"],
        ...
        rows_by_coordinate.setdefault(coordinate, []).append(row)
    for coordinate, rows in rows_by_coordinate.items():
        if coordinate[-1] != "flow_branch_label":
            assert len(rows) == 1
            assert rows[0]["semantic_ordinal_at_span"] == 0
            continue
        ...
        encoded_paths = [
            [node.encode("utf-8") for node in row["flow_branch_paths"][0]]
            for row in rows
        ]
        assert encoded_paths == sorted(encoded_paths)
