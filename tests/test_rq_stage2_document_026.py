"""Independent validation of the sealed R_Q document-26 annotation.

Every row assertion below is re-derived from ``q80.pdf`` through the
...
SOURCE_PDF = CAPTURE_ROOT / "q80.pdf"
ANNOTATION_PATH = (
    ROOT
    / "docs"
    / "analysis"
    / "rq_stage2_annotations"
    / "document_026_q80_annotation_v1.json"
)

if str(SCRIPTS) not in sys.path:
    sys.path.insert(0, str(SCRIPTS))

import build_rq_stage2_document_026_annotation as annotation  # noqa: E402
...
FILE_SIZE = 2_335_179
FILE_SHA256 = (
    "8044c2327d7c0ffaeff0e4183ee79f90694c7820c58078d89702589c1acf6b38"
)
...
def test_whole_document_locator_obeys_every_equation(sealed: dict) -> None:
    ...
    assert locator["filename"] == "q80.pdf"
...
def test_empty_occurrence_pages_are_still_emitted(sealed: dict) -> None:
    ...
    assert len(empty) == PAGE_COUNT - 27
    assert {
