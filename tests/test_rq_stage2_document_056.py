"""Validation and mutation coverage for the sealed fam1996_QxQs shard."""
...
import build_rq_stage2_document_056 as builder  # noqa: E402
...
def _capture_root() -> Path:
    ...
    if not (capture_root / builder.PDF_FILENAME).exists():
        pytest.skip("staged PSID fam1996_QxQs.pdf is unavailable")
    return capture_root
...
def test_document_056_reproduces_committed_sealed_annotation():
...
def test_document_056_committed_shard_verifies_against_source_bytes():
...
def test_document_056_mutations_fail_closed():
    capture_root = _capture_root()
    ...
    # Every fam1996_QxQs occurrence resolves under exactly one branch path,
    # so the path-subset law is exercised by truncating a conditional
    # occurrence's complete root-to-leaf path back to the bare root rather
    # than by dropping one member of a multi-parent path array.
...
def test_document_056_nested_branch_ancestry_is_exact():
    """The 1996 shard nests Section C inside the Sections B/C scope box."""
    ...
    nested = [
        row
        for row in branches
        if row["parent_flow_branch_id"] != "questionnaire-flow:root"
    ]
    assert nested, "the shard must retain at least one nested branch"
    for row in nested:
        parent = by_id[row["parent_flow_branch_id"]]
        assert row["branch_path"] == parent["branch_path"] + [
            row["flow_branch_id"]
        ]
        parent_source = occurrences[parent["source_occurrence_id"]]
        child_source = occurrences[row["source_occurrence_id"]]
        assert (
            parent_source["interview_wave"] == child_source["interview_wave"]
        )
        assert (
            parent_source["page_number"],
            parent_source["utf8_byte_start"],
        ) < (
            child_source["page_number"],
            child_source["utf8_byte_start"],
        )

    depth_two = [row for row in branches if len(row["branch_path"]) == 3]
    assert depth_two, "Section C must resolve two levels below the root"

    cyclic = copy.deepcopy(committed)
    target = next(
        row
        for row in cyclic["flow_branch_rows"]
        if row["parent_flow_branch_id"] != "questionnaire-flow:root"
    )
