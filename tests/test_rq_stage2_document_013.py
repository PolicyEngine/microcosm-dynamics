"""Independent validation of the sealed R_Q document-13 annotation.

Every row assertion below is re-derived from ``fam1974_QxQs.pdf`` through the
...
SOURCE_PDF = CAPTURE_ROOT / "fam1974_QxQs.pdf"
ANNOTATION_PATH = (
    ROOT
    / "docs"
    / "analysis"
    / "rq_stage2_annotations"
    / "document_013_fam1974_QxQs_annotation_v1.json"
)
...
INTERVIEW_WAVE = 1974
PAGE_COUNT = 62
FILE_SIZE = 18_785_798
FILE_SHA256 = (
    "bc9c5f58c3d12a3108d53900936783c090e0cd5dce8124e8b89666252afb0cd6"
)
...
def test_whole_document_locator_obeys_every_equation(sealed: dict) -> None:
    ...
    assert locator["filename"] == "fam1974_QxQs.pdf"
...
KNOWN_EMPTY_PAGES = (
    frozenset(range(1, 9))
    | {28, 31, 32, 36, 42, 52}
    | frozenset(range(55, 63))
)
KNOWN_INSTRUMENT_PAGES = frozenset(
    {9, 13, 15, 19, 23, 25, 27, 29, 33, 37, 43, 51, 53}
)
...
def test_atomic_span_kind_ordinals_are_unique_and_contiguous(
    ...
    multiparent = [
        row
        for row in sealed["questionnaire_occurrence_rows"]
        if row["matched_text"] == "(IF YES TO F2 OR TO F7)"
    ]
...
def test_source_review_is_complete_sealed_input(
    ...
    assert review["review_method"]["whole_page_review"] == (
        "all_62_pages_including_empty_occurrence_pages"
    )
...
def test_document_013_retained_and_rejected_boundaries(
    sealed: dict, page_texts: list[str]
) -> None:
    ...
    assert counts["role_total_anchor"] == 3
    ...
    assert {
        ("farm_aggregate_anchor", 33, "net income from farming"),
        ("business_aggregate_anchor", 33, "unincorporated business"),
    } <= aggregates
    ...
    # Page 49 changes from the income schedule to support, background, and
    # union questions at H36. Nothing at or after that exact boundary belongs
    # in this document's R_Q annotation.
    page_49_raw = page_texts[48].encode("utf-8")
    h36_start = page_49_raw.index(b"H36 .")
    assert by_page[49]
    assert all(row["utf8_byte_end"] <= h36_start for row in by_page[49])
