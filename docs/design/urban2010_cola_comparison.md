...
- **Specification:** `urban2010_cola_exercise1`, version `a1-draft-1`,
  drafted 2026-09-22.
...
- **Starting point:** the plan's §4 table "Proposed specification entries
  for A1". §20 lists every place this draft adds to or departs from that
  table.
...
- **Builder boundary:** a model-builder lane wrote this draft. It did not
  open the sealed comparator directory, the seal file, Figure 2 or page 3
  of the Urban report, and it did not look up Urban or DYNASIM results for
  this exercise. §2 lists what it read.
...
against the figure, by design (§23, §24).
...
|---|---|---|---|
| Urban (2010), text extraction | `EVID/urban-2010-reform-details.txt` | `0826fc19…` | Lines 44–55 (page-1 footnotes), 56–90 (Table 1), 125–126 (outcome sentence), 160 (source line), 414–418 (references) |
...
| 2008 OASDI Trustees Report (TR2008) | `EVID/tr2008-inputs-20260922/tr08-2008-oasdi-trustees-report.pdf` | `517de81a…` | Re-parsed with `pdftotext -layout`; pp. 31–32, 55, 75, 85–86, 89, 92–93, 100–106, 163–164, 182–185 |
...
| Realized SSA COLA series | `data/external/ssa_cola_history.json` | `12da7f0e…` | Determination-year values 2007–2022 |
| Repo code, for conventions only | `estimates/ledgers.py` (`_monthly_benefit_path`), `estimates/parameters.py` (`REPORT_YEARS`), `engine/rng.py` (`DRAW_SEED_BASE`), `estimates/first_report.py` (draw aggregates), `harness/panel.py` (`split_panel_by_person`), `ss/__init__.py`, `ss/benefits.py` (`spousal_benefit`, `widow_benefit`) | — | Read this session |
| PSID setup files, labels only | `~/PolicyEngine/psid-data/ind2023er/IND2023ER.sas`, `~/PolicyEngine/psid-data/family/2011/FAM2011ER.sas` | — | Labels of ER34046, ER34154, ER34155 and ER52337–ER52342. No PSID values were read |
...
**Disclosure.** A line-number search of the Urban text extraction printed
the opening clause of line 164 (page 3). The clause refers to Figure 2
qualitatively and contains no number. No field in this document relies on
it.
...
5. Reduced increases apply to every benefit in force at each reduced
   increase, including benefits of people on the rolls before 2010.
   Increases before the first reduced increase stay as they were. Table 1
   states no exemption, and 415(i)(2)(A)(ii) applies each increase to
   each amount "as previously increased" (line 55).
...
20.0 percent (lines 29–31). The TR2008 intermediate OASDI fund ratio is
378 (2010), 385 (2017), 361 (2020), 302 (2025) and 221 (2030)
(Table IV.B3, p. 55); no listed year from 2008 to 2030 is below 221. The
table omits the years between those points; this draft infers that they
also stay far above 20. The applicable increase is therefore the CPI
...
- Levels that act only as weights may be observed. The opening stock's
  2010 Social Security amounts (§14) embed the realized December 2008
  (5.8 percent) and December 2009 (0.0 percent) increases. New entrants'
  PIAs, computed by the oracle, carry the TR2008 values (2.7 and 2.5).
  The resulting level difference, 1.058 / (1.027 × 1.025) − 1 ≈ +0.5
  percent for the opening stock, is a named delta (§12).
...
payment. R0 reads it in payment terms. R1 reads it as the first increase that takes
effect in calendar 2010. The one-month gap between effect and payment is
statutory (415(i)(2)(B)); the repo's realized series records the same
timing ("effective in December of the determination year and first
reflected in January payments of the following year",
`ssa_cola_history.json` `historical_timing`). The plan also reports that
SSA's TR2008 A1 provision starts in December 2009; this draft did not
verify that and does not rely on it.
...
death), not the year of first benefit receipt. On p. 7 it notes that
spouses of older retirees often receive benefits resting on a benefit
computation made several years before they reach retirement age. The
...
increases in 2030 payments. Opening-stock clocks:

- Retired workers: birth year + 62.
- DI and survivor beneficiaries under 62 in 2010: A3 freezes a rule that
  places the start of receipt no earlier than the observations allow, so
  the count is never overstated. The understatement is at most one
  increase, for people first observed receiving in 2010.
...
dollar-weighted aggregate as a delta (`named_population_deltas`).
...
statistic. The run reports it as undefined and never imputes it.
...
- `B[i]` is the monthly benefit for December 2030, which includes the
  increase for determination year 2030 (415(i)(2)(B)). So `last` = 2030,
  and every count rises by one (22 under R0 for a clock started in 2009
  or earlier).
...
**Alternative (R5):** retired and disabled workers only.
...
4. **Opening stock** (persons with observed 2010 Social Security income;
   §14): the monthly PIA is not observed, so
   `B_base = observed 2010 annual amount × Π_{t=2010..last} (1 + c_t)`
   and `B_reform = B_base × Π_{t∈T_i} (1 + c_t − 0.01) / (1 + c_t)`,
   where `T_i` is the person's set of reduced determination years. No
   dime flooring applies. In the worked cases of §19 the flooring changes
   the percentage by less than 0.02 points.
5. **Gross benefits:** amounts are before Medicare premium deductions,
   income tax, earnings-test withholding and WEP/GPO, all omitted (§12).
   Whole-dollar payment rounding is omitted.
...
| Opening-stock level splice | — | Opening-stock levels embed realized 2008–2009 increases, about +0.5 percent against new entrants (§4). Weights only |
| Benefit type unobserved in PSID 2010 | — | A3's classification rule for under-62 recipients. Membership by component, and the clock of DI versus survivor records |
...
- The 2009 wave matches DYNASIM's 2008 information date (plan §3). The
  only realized increase embedded in its levels is determination year
  2007, and TR2008 reports that year as actual (2.3, V.C1 fn. 7), so R6
  has no level splice.
...
| DI incidence and termination by age and sex | Membership, and counts for DI and survivor records | Aggregate assumptions and sensitivity tests only (VI.D7–VI.D8, pp. 163–164) | A2/A4 substitute, fit on data through 2008 (plan A4) |
...
- **Half-split floor:**
  - seeds 0–4, fraction 0.5, using `harness/panel.split_panel_by_person`
    on the 2011 family-unit identifier, so each family unit's persons fall
    on one side;
  ...
  `replication_mermin_rows_v1.json`, except for the split unit (§23).
...
| R4 | Benefit period | December 2030 monthly amount | No |
...
| `population_vintage_coverage_and_weights` | R0: PSID 2011 wave, born ≤ 1980, ER34155, closed cohort. R6: 2009 wave | §14 |
...
| `benefit_period_and_partial_year_rules` | R0: 2030 payments, full year; R4: December 2030 amount | §10 |
| `benefit_components_offsets_and_rounding` | Five components (R5: workers only); dime flooring; oracle dual entitlement; gross benefits | §11, §12 |
...
```json
{
  ...
  "uncertainty": {
    ...
    "floor": {
      "seeds": [0, 1, 2, 3, 4],
      "fraction": 0.5,
      "split_unit": "psid_2011_family_unit",
      "summary": ["mean", "sd"]
    },
...
2. **Scope of the transitional oracle.** `ss/__init__.py` says "Do not
   ...
   - **(b)** (`di_benefit_level`) May the DI benefit level be a disclosed
     ...
     **Consequence of the alternative:** new DI awards from 2011 to 2030
     would have no level. Disabled workers then could not enter the
     primary statistic, the 50–61 cell would lose most of its members,
     and the disability-inclusive target could not be scored.
...
2. **Floor split unit.** Is keeping 2011 family units whole in the
   half-split an acceptable departure from the person-level split of the
   Mermin-row precedent? Spouse links drive widowhood in Track A.
