#!/usr/bin/env python3
"""Build and validate the stage-2 nonauthority annotation for document 26.
...
REVIEW_PATH = ANNOTATION_ROOT / "document_026_q80_source_review_v1.json"
OUTPUT_PATH = ANNOTATION_ROOT / "document_026_q80_annotation_v1.json"
CAPTURE_ROOT = (
...
def _document_identity(
    ...
    if (
        ...
        raise ValueError("document 26 candidate-index resolution drift")
...
def _extract_page_texts(
    ...
    if questionnaire_inventory._pdftotext_version() != "26.04.0":
        raise ValueError("document 26 Poppler version drift")
    ...
    if len(page_texts) != 59 or len(page_texts) != len(expected):
        raise ValueError("document 26 page denominator drift")
    for page_text, page_row in zip(page_texts, expected, strict=True):
        ...
        if (
            ...
            raise ValueError("document 26 page replay drift")
    return page_texts
...
def _load_review(
    ...
    review = _strict_json(REVIEW_PATH, "document 26 source review")
...
def _load_candidates(
    ...
    if (
        ...
        raise ValueError("document 26 candidate raw identity drift")
    value = source_tools.strict_parse_document(raw, "document 26 candidates")
    if not isinstance(value, dict):
        raise ValueError("document 26 candidates are not an object")
    ...
    if (
        ...
        raise ValueError("document 26 candidate content identity drift")
    return value
...
def validate_review(
    ...
    for spec in occurrence_specs:
        ...
        last_order = order
    # The independent document-26 source ledger selects one semantic atom at
        ...
        for position, left in enumerate(same_kind_specs):
            for right in same_kind_specs[position + 1 :]:
                if right["utf8_byte_start"] >= left["utf8_byte_end"]:
                    break
                raise ValueError(
                    "document 26 source review has partially overlapping "
                    "same-kind atoms"
                )
    branch_specs = [
        spec
...
def _mutation_specs(value: Mapping[str, Any]) -> list[tuple[str, Any]]:
    ...
    def select_path_subset(row: dict[str, Any]) -> None:
        ...
        Every document-26 occurrence resolves under exactly one complete
