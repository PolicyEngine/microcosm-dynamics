#!/usr/bin/env python3
"""Build and validate the stage-2 nonauthority annotation for document 38.
...
REVIEW_PATH = ANNOTATION_ROOT / "document_038_q86_source_review_v1.json"
OUTPUT_PATH = ANNOTATION_ROOT / "document_038_q86_annotation_v1.json"
CAPTURE_ROOT = (
...
def _document_identity(
    ...
    if (
        ...
        raise ValueError("document 38 candidate-index resolution drift")
...
def _extract_page_texts(
    ...
    if questionnaire_inventory._pdftotext_version() != "26.04.0":
        raise ValueError("document 38 Poppler version drift")
    ...
    if len(page_texts) != 59 or len(page_texts) != len(expected):
        raise ValueError("document 38 page denominator drift")
    for page_text, page_row in zip(page_texts, expected, strict=True):
        ...
        if (
            ...
            raise ValueError("document 38 page replay drift")
    return page_texts
...
def _load_review(
    ...
    review = _strict_json(REVIEW_PATH, "document 38 source review")
...
def _load_candidates(
    ...
    if (
        ...
        raise ValueError("document 38 candidate raw identity drift")
    value = source_tools.strict_parse_document(raw, "document 38 candidates")
    if not isinstance(value, dict):
        raise ValueError("document 38 candidates are not an object")
    ...
    if (
        ...
        raise ValueError("document 38 candidate content identity drift")
    return value
...
def validate_review(
    ...
    for spec in occurrence_specs:
        ...
        last_order = order
    # The independent document-38 source ledger selects one semantic atom at
        ...
        for position, left in enumerate(same_kind_specs):
            for right in same_kind_specs[position + 1 :]:
                if right["utf8_byte_start"] >= left["utf8_byte_end"]:
                    break
                raise ValueError(
                    "document 38 source review has partially overlapping "
                    "same-kind atoms: "
                    f"page {left['page_number']} "
                    f"{left['occurrence_kind']} "
                    f"{left['utf8_byte_start']}:{left['utf8_byte_end']} and "
                    f"{right['utf8_byte_start']}:{right['utf8_byte_end']}"
                )
    branch_specs = [
        spec
        for spec in occurrence_specs
...
def _validate_compatibility_predicate() -> None:
    ...
    root_row = {"interview_wave": 1986, "flow_branch_paths": [root]}
    left_or_right = {
        "interview_wave": 1986,
    ...
    selected_left_only = {
        "interview_wave": 1986,
        "flow_branch_paths": [left],
    }
    left_row = {"interview_wave": 1986, "flow_branch_paths": [left]}
    right_row = {"interview_wave": 1986, "flow_branch_paths": [right]}
    other_wave = {"interview_wave": 1985, "flow_branch_paths": [left]}
    if (
        not _branch_compatible([root_row, left_row], domain)
        or not _branch_compatible([left_or_right, left_row], domain)
        or not _branch_compatible([left_or_right, right_row], domain)
        or _branch_compatible([selected_left_only, right_row], domain)
...
def _mutation_specs(value: Mapping[str, Any]) -> list[tuple[str, Any]]:
    ...
    if value["flow_branch_rows"]:
        ...
        if any(
            len(occurrence["flow_branch_paths"]) > 1
...
def _run_synthetic_selected_path_subset_mutation(
    inputs: tuple[
        dict[str, Any],
        dict[str, Any],
        dict[str, Any],
    ...
    """Exercise subset rejection when the reviewed document has no multipath row."""

    review = copy.deepcopy(inputs[5])
    root_branch_review_ids = [
        spec["review_occurrence_id"]
        for spec in review["occurrence_specs"]
        if spec["occurrence_kind"] == "flow_branch_label"
        and spec["parent_review_branch_paths"] == [[]]
    ]
    if len(root_branch_review_ids) < 2:
        raise ValueError(
            "selected-path-subset fallback has no sibling branches"
        )
    first_branch, second_branch = root_branch_review_ids[:2]
    target_spec = next(
        (
            spec
            for spec in review["occurrence_specs"]
            if spec["occurrence_kind"] == "field_purpose_prompt"
            and spec["parent_review_branch_paths"] == [[first_branch]]
        ),
        None,
    )
    if target_spec is None:
        raise ValueError("selected-path-subset fallback has no source prompt")
    target_coordinate = (
        target_spec["page_number"],
        target_spec["utf8_byte_start"],
        target_spec["utf8_byte_end"],
        target_spec["occurrence_kind"],
    )
    target_spec["parent_review_branch_paths"] = [
        [first_branch],
        [second_branch],
    ]
    review["integrity"]["content_sha256"] = _content_sha256(review)
    validate_review(review, inputs[2], inputs[4])
    synthetic_inputs = (
        inputs[0],
        inputs[1],
        inputs[2],
    ...
    synthetic = build_annotation(*synthetic_inputs)
    target_occurrence = next(
        (
            occurrence
            for occurrence in synthetic["questionnaire_occurrence_rows"]
            if (
                occurrence["page_number"],
                occurrence["utf8_byte_start"],
                occurrence["utf8_byte_end"],
    ...
    if (
        target_occurrence is None
        or len(target_occurrence["flow_branch_paths"]) != 2
    ):
        raise ValueError(
            "selected-path-subset fallback did not build multipath row"
        )
    target_occurrence["flow_branch_paths"] = target_occurrence[
        "flow_branch_paths"
    ][:-1]
    synthetic["integrity"]["content_sha256"] = _content_sha256(synthetic)
    try:
        validate_annotation(synthetic, *synthetic_inputs)
    except ValueError:
        return
    raise ValueError("mutation was not rejected: selected_path_subset")
...
def run_mutation_tests(
    ...
    for source_spec in review_mutation["occurrence_specs"]:
        source_kind = source_spec["occurrence_kind"]
        parent_paths = source_spec["parent_review_branch_paths"]
        is_root_flow_label = (
            source_kind == "flow_branch_label" and parent_paths == [[]]
        )
    ...
    if not any(
        ...
        _run_synthetic_selected_path_subset_mutation(inputs)
