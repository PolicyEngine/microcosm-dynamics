#!/usr/bin/env python3
"""Build and validate the stage-2 nonauthority annotation for document 35.
...
REVIEW_PATH = (
    ANNOTATION_ROOT / "document_035_fam1985_QxQs_source_review_v1.json"
)
OUTPUT_PATH = ANNOTATION_ROOT / "document_035_fam1985_QxQs_annotation_v1.json"
CAPTURE_ROOT = (
...
def _document_identity(
    ...
    if (
        ...
        raise ValueError("document 35 candidate-index resolution drift")
...
def _extract_page_texts(
    ...
    if questionnaire_inventory._pdftotext_version() != "26.04.0":
        raise ValueError("document 35 Poppler version drift")
    ...
    if len(page_texts) != 199 or len(page_texts) != len(expected):
        raise ValueError("document 35 page denominator drift")
    for page_text, page_row in zip(page_texts, expected, strict=True):
        ...
        if (
            ...
            raise ValueError("document 35 page replay drift")
    return page_texts
...
def _load_review(
    ...
    review = _strict_json(REVIEW_PATH, "document 35 source review")
...
def _load_candidates(
    ...
    if (
        ...
        raise ValueError("document 35 candidate raw identity drift")
    value = source_tools.strict_parse_document(raw, "document 35 candidates")
    if not isinstance(value, dict):
        raise ValueError("document 35 candidates are not an object")
    ...
    if (
        ...
        raise ValueError("document 35 candidate content identity drift")
    return value
...
def validate_review(
    ...
    for spec in occurrence_specs:
        ...
        last_order = order
    # The independent document-35 source ledger selects one semantic atom at
        ...
        for position, left in enumerate(same_kind_specs):
            for right in same_kind_specs[position + 1 :]:
                if right["utf8_byte_start"] >= left["utf8_byte_end"]:
                    break
                raise ValueError(
                    "document 35 source review has partially overlapping "
                    "same-kind atoms"
                )
    branch_specs = [
        spec
