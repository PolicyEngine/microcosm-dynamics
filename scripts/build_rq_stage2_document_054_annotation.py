#!/usr/bin/env python3
"""Build and validate the stage-2 nonauthority annotation for document 54.
...
SOURCE_PRINTED_IDENTIFIER_RE = re.compile(
    r"^\s*((?:[A-Z]{1,10}\d[A-Za-z0-9_.-]{0,22})|"
    r"(?:[A-Z][A-Za-z0-9_]{2,32}))\s*\."
)
SOURCE_PRINTED_IDENTIFIER_ONLY_RE = re.compile(
    r"^\s*((?:[A-Z]{1,10}\d[A-Za-z0-9_.-]{0,22})|"
    r"(?:[A-Z][A-Za-z0-9_]{2,32}))\s*\.\s*$"
)

ANNOTATION_ROOT = ROOT / "docs" / "analysis" / "rq_stage2_annotations"
REVIEW_PATH = (
    ANNOTATION_ROOT / "document_054_fam1995_QxQs_source_review_v1.json"
)
OUTPUT_PATH = ANNOTATION_ROOT / "document_054_fam1995_QxQs_annotation_v1.json"
CAPTURE_ROOT = (
...
def _document_identity(
    ...
    if (
        ...
        raise ValueError("document 54 candidate-index resolution drift")
...
def _extract_page_texts(
    ...
    if questionnaire_inventory._pdftotext_version() != "26.04.0":
        raise ValueError("document 54 Poppler version drift")
    ...
    if len(page_texts) != 69 or len(page_texts) != len(expected):
        raise ValueError("document 54 page denominator drift")
    for page_text, page_row in zip(page_texts, expected, strict=True):
        ...
        if (
            ...
            raise ValueError("document 54 page replay drift")
    return page_texts
...
def _load_review(
    ...
    review = _strict_json(REVIEW_PATH, "document 54 source review")
...
def _load_candidates(
    ...
    if (
        ...
        raise ValueError("document 54 candidate raw identity drift")
    value = source_tools.strict_parse_document(raw, "document 54 candidates")
    if not isinstance(value, dict):
        raise ValueError("document 54 candidates are not an object")
    ...
    if (
        ...
        raise ValueError("document 54 candidate content identity drift")
    return value
...
def _source_printed_identifier(page_text: str, byte_start: int) -> str | None:
    lines = stage1_candidates._physical_lines(page_text)
    for position, line in enumerate(lines):
        ...
        if line_byte_start <= byte_start < line_byte_end:
            identifier = stage1_candidates._printed_identifier(line["text"])
            if identifier is None:
                match = SOURCE_PRINTED_IDENTIFIER_RE.search(line["text"])
                identifier = None if match is None else match.group(1)
            if identifier is not None:
                return identifier
            if position:
                previous = lines[position - 1]
                previous_end = len(
                    page_text[: previous["end"]].encode("utf-8")
                )
                match = SOURCE_PRINTED_IDENTIFIER_ONLY_RE.fullmatch(
                    previous["text"]
                )
                if match is not None and line_byte_start - previous_end <= 96:
...
def validate_review(
    ...
    if method != {
        ...
        "whole_page_review": "all_69_pages_including_empty_occurrence_pages",
    ...
    for spec in occurrence_specs:
        for path in spec["parent_review_branch_paths"]:
            ...
            for branch_ref in path:
                branch_spec = branch_ref_to_spec[branch_ref][0]
                if (
                    occurrence_order[branch_spec["review_occurrence_id"]]
                    >= occurrence_order[spec["review_occurrence_id"]]
                ):
                    raise ValueError(
                        "source review path has later branch: "
                        f"{spec['review_occurrence_id']} <- "
                        f"{branch_spec['review_occurrence_id']}"
                    )
...
def _match_candidate_occurrences(
    ...
    for output in occurrences:
        ...
        outputs_by_page_and_kind[
            (output["page_number"], output["occurrence_kind"])
        ].append(output)

    # Candidate rows project independently onto the source-authored domain.
    # Several detector fragments can legitimately correct to the same complete
    # source atom, so an output must never become unavailable merely because an
    # earlier candidate already projects to it.
    for candidate in candidate_rows:
        ...
        exact_outputs = outputs_by_coordinate.get(coordinate, [])
        if exact_outputs:
            matched_outputs = exact_outputs
            disposition = "accepted" if len(exact_outputs) == 1 else "split"
        else:
            matched_outputs = [
                output
                for output in outputs_by_page_and_kind.get(
                    (
                        candidate["page_number"],
                        candidate["occurrence_kind_candidate"],
                    ),
            ...
            disposition = (
                ...
                else "modified" if len(matched_outputs) == 1 else "split"
            )
        matched_outputs.sort(
            key=lambda row: (
                row["utf8_byte_start"],
                row["utf8_byte_end"],
        ...
        output_ids = [
            row["questionnaire_occurrence_id"] for row in matched_outputs
        ]
...
def _adjudicate(
    ...
    for branch in branches:
        ...
        if not candidate_ids:
            ...
            flow_candidate_disposition[candidate_id] == "split"
            for candidate_id in candidate_ids
        ):
            action = "candidate_split"
        elif all(
            flow_candidate_disposition[candidate_id] == "accepted"
            for candidate_id in candidate_ids
        ):
            action = "candidate_accepted"
        else:
...
def _validate_adjudication_relations(
        ...
        for output_id in disposition["stage2_row_ids"]:
            ...
            if key[1] not in output["source_candidate_ids"]:
                raise ValueError(
                    ...
                    f"candidate={key} output={output_key}"
                )
    ...
    # The occurrence projection is a complete many-to-many interval join, not
    # a greedy assignment.  In particular, a corrected source atom may be the
    # lawful target of several overlapping detector fragments.
    occurrence_outputs_by_coordinate: dict[
        tuple[Any, ...], list[Mapping[str, Any]]
    ] = defaultdict(list)
    occurrence_outputs_by_page_and_kind: dict[
        tuple[int, str], list[Mapping[str, Any]]
    ] = defaultdict(list)
    for occurrence in value["questionnaire_occurrence_rows"]:
        occurrence_outputs_by_coordinate[
            (
                occurrence["page_number"],
                occurrence["utf8_byte_start"],
                occurrence["utf8_byte_end"],
        ...
        occurrence_outputs_by_page_and_kind[
            (occurrence["page_number"], occurrence["occurrence_kind"])
        ].append(occurrence)
    expected_candidates_by_output: dict[str, list[str]] = defaultdict(list)
    expected_disposition_by_candidate: dict[str, str] = {}
    for candidate in candidates["candidate_occurrence_rows"]:
        candidate_id = candidate["candidate_occurrence_id"]
        coordinate = (
            candidate["page_number"],
        ...
        matched_outputs = occurrence_outputs_by_coordinate.get(coordinate, [])
        exact = bool(matched_outputs)
        if not exact:
            matched_outputs = [
                occurrence
                for occurrence in occurrence_outputs_by_page_and_kind.get(
                    (
                        candidate["page_number"],
                        candidate["occurrence_kind_candidate"],
                    ),
                ...
                if occurrence["utf8_byte_start"] < candidate["utf8_byte_end"]
                and candidate["utf8_byte_start"] < occurrence["utf8_byte_end"]
            ]
        matched_outputs = sorted(
            matched_outputs,
            key=lambda row: (
                row["utf8_byte_start"],
                row["utf8_byte_end"],
        ...
        expected_disposition = (
            "accepted"
            if exact and len(expected_output_ids) == 1
            else (
                "split"
                if len(expected_output_ids) > 1
                else "modified" if expected_output_ids else "rejected"
            )
        )
        serialized = disposition_by_key[("occurrence", candidate_id)]
        if (
            serialized["stage2_row_ids"] != expected_output_ids
            or serialized["disposition"] != expected_disposition
        ):
            raise ValueError("candidate occurrence interval projection drift")
        expected_disposition_by_candidate[candidate_id] = expected_disposition
        for output_id in expected_output_ids:
            expected_candidates_by_output[output_id].append(candidate_id)
    for occurrence in value["questionnaire_occurrence_rows"]:
        output_id = occurrence["questionnaire_occurrence_id"]
        candidate_ids = expected_candidates_by_output.get(output_id, [])
        serialized = output_by_key[("occurrence", output_id)]
        expected_action = (
            "manual_add"
            if not candidate_ids
            else (
                "candidate_accepted"
                if {
                    expected_disposition_by_candidate[candidate_id]
                    for candidate_id in candidate_ids
                }
                == {"accepted"}
                else (
        ...
        if (
            serialized["source_candidate_ids"] != candidate_ids
            or serialized["adjudication_action"] != expected_action
        ):
            raise ValueError("output occurrence interval projection drift")
...
def _validate_compatibility_predicate() -> None:
    ...
    root_row = {"interview_wave": 1994, "flow_branch_paths": [root]}
    left_or_right = {
        "interview_wave": 1994,
        "flow_branch_paths": [left, right],
    }
    left_row = {"interview_wave": 1994, "flow_branch_paths": [left]}
    right_row = {"interview_wave": 1994, "flow_branch_paths": [right]}
...
def _validate_local_anchor_laws(value: Mapping[str, Any]) -> None:
    ...
    for anchor in value["local_anchor_classification_rows"]:
        ...
        if (
            ...
            or any(
                parent["page_number"] != source["page_number"]
                or not (
    ...
    def select_path_subset(row: dict[str, Any]) -> None:
        for occurrence in row["questionnaire_occurrence_rows"]:
            if len(occurrence["flow_branch_paths"]) > 1:
                ...
                return
        # Document 54 prints no multi-parent label, so the subset mutation
        # truncates a resolved conditional path to a proper ancestor prefix.
        # That is the same defect: a lawful complete path replaced by a
        # selected subset of the printed condition.
        for occurrence in row["questionnaire_occurrence_rows"]:
            if len(occurrence["flow_branch_paths"][0]) > 1:
                occurrence["flow_branch_paths"] = [
                    occurrence["flow_branch_paths"][0][:-1]
                ]
                return
        raise ValueError("mutation fixture has no resolvable path subset")
