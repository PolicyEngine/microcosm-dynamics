#!/usr/bin/env python3
"""Build and validate the stage-2 nonauthority annotation for document 78.
...
REVIEW_PATH = ANNOTATION_ROOT / "document_078_q2017_source_review_v1.json"
OUTPUT_PATH = ANNOTATION_ROOT / "document_078_q2017_annotation_v1.json"
CAPTURE_ROOT = (
...
def _document_identity(
    ...
    if (
        ...
        raise ValueError("document 78 candidate-index resolution drift")
...
def _extract_page_texts(
    ...
    if questionnaire_inventory._pdftotext_version() != "26.04.0":
        raise ValueError("document 78 Poppler version drift")
    ...
    if len(page_texts) != 288 or len(page_texts) != len(expected):
        raise ValueError("document 78 page denominator drift")
    for page_text, page_row in zip(page_texts, expected, strict=True):
        ...
        if (
            ...
            raise ValueError("document 78 page replay drift")
    return page_texts
...
def _load_review(
    ...
    review = _strict_json(REVIEW_PATH, "document 78 source review")
...
def _load_candidates(
    ...
    if (
        ...
        raise ValueError("document 78 candidate raw identity drift")
    value = source_tools.strict_parse_document(raw, "document 78 candidates")
    if not isinstance(value, dict):
        raise ValueError("document 78 candidates are not an object")
    ...
    if (
        ...
        raise ValueError("document 78 candidate content identity drift")
    return value
...
def validate_review(
    ...
    if method != {
        ...
        "whole_page_review": "all_288_pages_including_empty_occurrence_pages",
...
def _match_candidate_occurrences(
    ...
    for candidate in unmatched_candidates:
        ...
        overlaps = [
            output
            for output in occurrences
            if output["page_number"] == candidate["page_number"]
            and output["occurrence_kind"]
            == candidate["occurrence_kind_candidate"]
            and output["utf8_byte_start"] < candidate["utf8_byte_end"]
