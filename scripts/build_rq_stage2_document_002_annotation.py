#!/usr/bin/env python3
"""Build and validate the stage-2 nonauthority annotation for document 2.
...
REVIEW_PATH = ANNOTATION_ROOT / "document_002_q68_source_review_v1.json"
OUTPUT_PATH = ANNOTATION_ROOT / "document_002_q68_annotation_v1.json"
CAPTURE_ROOT = (
...
def _document_identity(
    ...
    if (
        ...
        raise ValueError("document 2 candidate-index resolution drift")
...
def _extract_page_texts(
    ...
    if questionnaire_inventory._pdftotext_version() != "26.04.0":
        raise ValueError("document 2 Poppler version drift")
    ...
    if len(page_texts) != 32 or len(page_texts) != len(expected):
        raise ValueError("document 2 page denominator drift")
    for page_text, page_row in zip(page_texts, expected, strict=True):
        ...
        if (
            ...
            raise ValueError("document 2 page replay drift")
    return page_texts
...
def _load_review(
    ...
    review = _strict_json(REVIEW_PATH, "document 2 source review")
...
def _load_candidates(
    ...
    if (
        ...
        raise ValueError("document 2 candidate raw identity drift")
    value = source_tools.strict_parse_document(raw, "document 2 candidates")
    if not isinstance(value, dict):
        raise ValueError("document 2 candidates are not an object")
    ...
    if (
        ...
        raise ValueError("document 2 candidate content identity drift")
    return value
...
def validate_review(
    ...
    for spec in occurrence_specs:
        ...
        last_order = order
    # Branch labels remain atomic and nonoverlapping. Purpose prompts may
    # lawfully nest: a shared question-level amount instruction can support
    # several narrower, co-spanned component prompts on the same screen.
    for (
        page_number,
        kind,
    ), same_kind_specs in review_specs_by_page_kind.items():
        ...
        for position, left in enumerate(same_kind_specs):
            for right in same_kind_specs[position + 1 :]:
                if right["utf8_byte_start"] >= left["utf8_byte_end"]:
                    break
                raise ValueError(
                    "document 2 source review has overlapping flow labels "
    ...
    for candidate in candidates["candidate_occurrence_rows"]:
        ...
        if disposition == "rejected":
            if (
                ...
                and "REPEAT J17-J27" in candidate["matched_text"]
            ):
                note_code = (
                    "repeat_targets_excluded_other_family_member_domain"
                )
                note = (
                    "Page 22 genuinely repeats the excluded J17-J27 "
                    "other-family-member block. The fixed head/spouse R_Q "
                    "projection has no lawful local alias endpoints, so the "
                    "instruction is consumed as a rejection and emits no "
                    "repeat or alias row."
                )
            else:
...
def _validate_compatibility_predicate() -> None:
    ...
    root_row = {"interview_wave": 1968, "flow_branch_paths": [root]}
    left_or_right = {
        "interview_wave": 1968,
        "flow_branch_paths": [left, right],
    }
    left_row = {"interview_wave": 1968, "flow_branch_paths": [left]}
    right_row = {"interview_wave": 1968, "flow_branch_paths": [right]}
    other_wave = {"interview_wave": 1970, "flow_branch_paths": [left]}
    if (
...
def run_mutation_tests(
    ...
    try:
        ...
        if "overlapping flow labels" not in str(error):
            raise ValueError(
