#!/usr/bin/env python3
"""Build and validate the stage-2 nonauthority annotation for document 16.
...
REVIEW_PATH = ANNOTATION_ROOT / "document_016_q75_source_review_v1.json"
OUTPUT_PATH = ANNOTATION_ROOT / "document_016_q75_annotation_v1.json"
CAPTURE_ROOT = (
...
def _document_identity(
    ...
    if (
        ...
        raise ValueError("document 16 candidate-index resolution drift")
...
def _extract_page_texts(
    ...
    if questionnaire_inventory._pdftotext_version() != "26.04.0":
        raise ValueError("document 16 Poppler version drift")
    ...
    if len(page_texts) != 29 or len(page_texts) != len(expected):
        raise ValueError("document 16 page denominator drift")
    for page_text, page_row in zip(page_texts, expected, strict=True):
        ...
        if (
            ...
            raise ValueError("document 16 page replay drift")
    return page_texts
...
def _load_review(
    ...
    review = _strict_json(REVIEW_PATH, "document 16 source review")
...
def _load_candidates(
    ...
    if (
        ...
        raise ValueError("document 16 candidate raw identity drift")
    value = source_tools.strict_parse_document(raw, "document 16 candidates")
    if not isinstance(value, dict):
        raise ValueError("document 16 candidates are not an object")
    ...
    if (
        ...
        raise ValueError("document 16 candidate content identity drift")
    return value
...
def _source_printed_identifier(
    page_text: str, byte_start: int, _matched_text: str | None = None
) -> str | None:
    """Resolve the nearest source-local printed identifier on its line."""
    ...
    for line in stage1_candidates._physical_lines(page_text):
        ...
        if line_byte_start <= byte_start < line_byte_end:
            ...
            # Multi-column text can place multiple identifiers on one line.
            # Search backward from the span and retain the nearest eligible
            # identifier, including the scan's whitespace-before-period OCR.
...
def validate_review(
    ...
    for spec in occurrence_specs:
        ...
        last_order = order
    # The independent document-16 source ledger selects one semantic atom at
        ...
        for position, left in enumerate(same_kind_specs):
            for right in same_kind_specs[position + 1 :]:
                if right["utf8_byte_start"] >= left["utf8_byte_end"]:
                    break
                raise ValueError(
                    "document 16 source review has partially overlapping "
                    "same-kind atoms"
                )
    branch_specs = [
        spec
...
def _mutation_specs(value: Mapping[str, Any]) -> list[tuple[str, Any]]:
    ...
    def select_path_subset(row: dict[str, Any]) -> None:
        ...
        Prefer a multi-parent non-flow occurrence and delete one complete
        path. If no such fixture survives a future source revision, truncate
        one conditional single path back to the root. Either mutation removes
        part of the reviewer-authored complete path projection and must be
        rejected.
        """
