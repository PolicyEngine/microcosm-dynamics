#!/usr/bin/env python3
"""Build and validate the stage-2 nonauthority annotation for document 73.
...
REVIEW_PATH = ANNOTATION_ROOT / "document_073_q2011_source_review_v1.json"
OUTPUT_PATH = ANNOTATION_ROOT / "document_073_q2011_annotation_v1.json"
CAPTURE_ROOT = (
...
def _document_identity(
    ...
    if (
        ...
        raise ValueError("document 73 candidate-index resolution drift")
...
def _extract_page_texts(
    ...
    if questionnaire_inventory._pdftotext_version() != "26.04.0":
        raise ValueError("document 73 Poppler version drift")
    ...
    if len(page_texts) != 205 or len(page_texts) != len(expected):
        raise ValueError("document 73 page denominator drift")
    for page_text, page_row in zip(page_texts, expected, strict=True):
        ...
        if (
            ...
            raise ValueError("document 73 page replay drift")
    return page_texts
...
def _load_review(
    ...
    review = _strict_json(REVIEW_PATH, "document 73 source review")
...
def _load_candidates(
    ...
    if (
        ...
        raise ValueError("document 73 candidate raw identity drift")
    value = source_tools.strict_parse_document(raw, "document 73 candidates")
    if not isinstance(value, dict):
        raise ValueError("document 73 candidates are not an object")
    ...
    if (
        ...
        raise ValueError("document 73 candidate content identity drift")
    return value
...
def validate_review(
    ...
    if method != {
        ...
        "whole_page_review": "all_205_pages_including_empty_occurrence_pages",
