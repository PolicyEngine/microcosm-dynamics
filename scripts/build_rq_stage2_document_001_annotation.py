#!/usr/bin/env python3
"""Build and validate the stage-2 nonauthority annotation for document 1.
...
REVIEW_PATH = (
    ANNOTATION_ROOT / "document_001_fam1968_QxQs_source_review_v1.json"
)
OUTPUT_PATH = ANNOTATION_ROOT / "document_001_fam1968_QxQs_annotation_v1.json"
CAPTURE_ROOT = (
...
def _document_identity(
    ...
    if (
        ...
        raise ValueError("document 1 candidate-index resolution drift")
...
def _extract_page_texts(
    ...
    if len(page_texts) != 50 or len(page_texts) != len(expected):
        raise ValueError("document 1 page denominator drift")
    for page_text, page_row in zip(page_texts, expected, strict=True):
        ...
        if (
            ...
            raise ValueError("document 1 page replay drift")
    return page_texts
...
def _load_review(
    ...
    review = _strict_json(REVIEW_PATH, "document 1 source review")
...
def _load_candidates(
    ...
    if (
        ...
        raise ValueError("document 1 candidate raw identity drift")
    value = source_tools.strict_parse_document(raw, "document 1 candidates")
    if not isinstance(value, dict):
        raise ValueError("document 1 candidates are not an object")
    ...
    if (
        ...
        raise ValueError("document 1 candidate content identity drift")
    return value
...
def _alias_root(alias_parents: dict[str, str], item: str) -> str:
    """Resolve one anchor through already-proved local alias facts."""

    parent = alias_parents[item]
    if parent != item:
        alias_parents[item] = _alias_root(alias_parents, parent)
    return alias_parents[item]


def _join_aliases(
    alias_parents: dict[str, str], alias_id: str, canonical_id: str
) -> None:
    alias_root = _alias_root(alias_parents, alias_id)
    canonical_root = _alias_root(alias_parents, canonical_id)
    if alias_root != canonical_root:
        alias_parents[alias_root] = canonical_root


def _endpoint_parents_resolve_equally(
    anchor_by_id: Mapping[str, Mapping[str, Any]],
    endpoint_ids: Sequence[str],
    parent_key: str,
    alias_parents: dict[str, str],
) -> bool:
    """Compare parent vectors after earlier explicit alias proofs."""

    vectors = [anchor_by_id[item][parent_key] for item in endpoint_ids]
    first = vectors[0]
    return all(
        len(vector) == len(first)
        and all(
            _alias_root(alias_parents, left)
            == _alias_root(alias_parents, right)
...
def validate_review(
    ...
    if method != {
        ...
        "whole_page_review": "all_50_pages_including_empty_occurrence_pages",
    ...
    for row in anchor_specs:
        ...
        if (
            occurrence["occurrence_kind"] == "remuneration_component_anchor"
            and len(row["parent_review_occurrence_ids"]) != 1
        ):
            raise ValueError(
                "source review remuneration component parent drift"
            )
        if (
            occurrence["occurrence_kind"] == "context_anchor"
            and len(row["parent_review_occurrence_ids"]) > 1
        ):
            raise ValueError("source review context parent drift")
    ...
    anchor_by_review_id = {
        row["review_occurrence_id"]: row for row in anchor_specs
    }
    alias_parents = {item: item for item in anchor_by_review_id}
    repeat_specs = review["repeat_alias_specs"]
    repeat_covered: set[str] = set()
    repeat_fact_keys: set[tuple[Any, ...]] = set()
    for row in repeat_specs:
        ...
        if (
            ...
            and (
                occurrence["occurrence_kind"] not in ALIAS_ANCHOR_KINDS
                or row["review_occurrence_id"] not in alias_ids
            )
            or row["relation"] != "same_printed_identifier_and_exact_label"
            and occurrence["occurrence_kind"] != "repeat_or_alias_instruction"
            or fact_key in repeat_fact_keys
        ...
        anchor_evidence = [
            item
            for item in evidence
            if spec_by_id[item]["occurrence_kind"] in ANCHOR_KINDS
        ]
        if resolution == "document_local_source_evidence_complete":
            if (
                ...
                or any(
                    spec_by_id[item]["occurrence_kind"]
                    not in ALIAS_ANCHOR_KINDS
                    for item in [*alias_ids, *canonical_ids]
                )
            ):
                raise ValueError(
                    "source review resolved alias cardinality drift"
                )
            classified_endpoint_ids = [*alias_ids, *canonical_ids]
            if not _endpoint_parents_resolve_equally(
                anchor_by_review_id,
                classified_endpoint_ids,
                "parent_review_occurrence_ids",
                alias_parents,
            ):
                raise ValueError("source review resolved alias parent drift")
        elif resolution == COMPOSITE_CROSS_REFERENCE_RESOLUTION:
            same_span_context_ids = {
                item
                for item in anchor_evidence
                if spec_by_id[item]["occurrence_kind"] == "context_anchor"
                and (
                    spec_by_id[item]["page_number"],
                    spec_by_id[item]["utf8_byte_start"],
                    spec_by_id[item]["utf8_byte_end"],
                )
                == (
            ...
            if (
                row["relation"] != "explicit_cross_reference"
                or occurrence["occurrence_kind"]
                != "repeat_or_alias_instruction"
                or alias_ids
                or canonical_ids
                or len(same_span_context_ids) != 1
                or len(set(anchor_evidence) - same_span_context_ids) < 2
                or any(
                    spec_by_id[item]["occurrence_kind"] != "context_anchor"
                    for item in anchor_evidence
                )
            ):
                raise ValueError(
                    "source review composite range evidence drift"
                )
            classified_endpoint_ids = anchor_evidence
        else:
            raise ValueError("source review repeat resolution status drift")
        endpoint_classifications = {
            (
                anchor_by_review_id[item]["node_domain"],
                anchor_by_review_id[item]["classification"],
            )
            for item in classified_endpoint_ids
        }
        if len(endpoint_classifications) != 1:
            raise ValueError("source review repeat endpoint type drift")
        ...
        repeat_fact_keys.add(fact_key)
        if resolution == "document_local_source_evidence_complete":
            _join_aliases(alias_parents, alias_ids[0], canonical_ids[0])
        if occurrence["occurrence_kind"] == "repeat_or_alias_instruction":
...
def _occurrence_rejection_note(
    ...
    if page_number == 2:
        return (
            ...
            (
                "Page 2 whole-page review rejected this housing valuation or "
                "home-production candidate: the source supplies neither a "
                "fixed-role earnings anchor nor a source-job parent, so it "
                "cannot form a lawful covered-earnings hierarchy node."
            ),
        )
    if (
        page_number == 34
        and candidate["utf8_byte_start"] < 1_710
        and 1_229 < candidate["utf8_byte_end"]
    ):
        return (
            "all_family_pay_in_kind_outside_fixed_role_job_domain",
            (
                "Page 34 whole-page review rejected this J30-31 pay-in-kind "
                "candidate because the question applies to anyone in the "
                "family and identifies neither of the two fixed earnings "
                "roles nor a source job; no lawful document-local parent can "
                "be resolved."
            ),
        )
    return (
        ...
        (
            f"Page {page_number} source review rejected the "
            f"{candidate['occurrence_kind_candidate']} candidate; the exact "
            "slice does not express that document-local section-19 "
            "occurrence kind."
        ),
    )
...
def _match_candidate_occurrences(
    ...
    for coordinate, coordinate_outputs in outputs_by_coordinate.items():
        ...
        for output_id in output_ids:
            ...
            unmatched_output_ids.remove(output_id)
    ...
    for candidate in unmatched_candidates:
        candidate_id = candidate["candidate_occurrence_id"]
        overlaps = [
            occurrence_by_id[output_id]
            for output_id in unmatched_output_ids
            if occurrence_by_id[output_id]["page_number"]
            == candidate["page_number"]
            and occurrence_by_id[output_id]["occurrence_kind"]
            == candidate["occurrence_kind_candidate"]
            and occurrence_by_id[output_id]["utf8_byte_start"]
            < candidate["utf8_byte_end"]
            and candidate["utf8_byte_start"]
            < occurrence_by_id[output_id]["utf8_byte_end"]
        ]
        overlaps.sort(
            key=lambda row: (
                row["utf8_byte_start"],
        ...
        for output_id in output_ids:
            output_to_candidates[output_id].append(candidate_id)
            unmatched_output_ids.discard(output_id)
...
def _adjudicate(
    ...
    for candidate in candidates["candidate_occurrence_rows"]:
        ...
        if disposition == "rejected":
            note_code, note = _occurrence_rejection_note(candidate)
            notes.append(
                _note_row(
                    "occurrence",
                    candidate_id,
...
def _validate_compatibility_predicate() -> None:
    ...
    root_row = {"interview_wave": 1972, "flow_branch_paths": [root]}
    left_or_right = {
        "interview_wave": 1972,
        "flow_branch_paths": [left, right],
    }
    left_row = {"interview_wave": 1972, "flow_branch_paths": [left]}
    right_row = {"interview_wave": 1972, "flow_branch_paths": [right]}
...
def _validate_local_anchor_laws(value: Mapping[str, Any]) -> None:
    ...
    for anchor in value["local_anchor_classification_rows"]:
        ...
        if (
            ...
            or source["occurrence_kind"] == "remuneration_component_anchor"
            and len(parents) != 1
            or source["occurrence_kind"] == "context_anchor"
            and len(parents) > 1
        ):
            raise ValueError("local anchor parent or path compatibility drift")


def _validate_local_repeat_laws(value: Mapping[str, Any]) -> None:
    occurrence_by_id = {
        row["questionnaire_occurrence_id"]: row
        for row in value["questionnaire_occurrence_rows"]
    }
    ...
    repeat_fact_keys: set[tuple[Any, ...]] = set()
    for row in value["local_repeat_alias_evidence_rows"]:
        source_id = row["source_occurrence_id"]
        ...
        if (
            ...
            or row["relation"] == "same_printed_identifier_and_exact_label"
            and (
                source["occurrence_kind"] not in ALIAS_ANCHOR_KINDS
                or source_id not in alias_ids
            )
            or row["relation"] != "same_printed_identifier_and_exact_label"
            and source["occurrence_kind"] != "repeat_or_alias_instruction"
            or fact_key in repeat_fact_keys
            or row["relation"] not in ALIAS_RELATIONS
            or row["target_scope"] != "document_local"
            or any(
                len(items) != len(set(items))
                for items in (alias_ids, canonical_ids, evidence_ids)
            )
            or set(alias_ids) & set(canonical_ids)
            or source_id not in evidence_ids
            or not {*alias_ids, *canonical_ids}.issubset(evidence_ids)
            or any(
                item not in anchor_by_source_id
                for item in [*alias_ids, *canonical_ids]
            )
            or any(item not in occurrence_by_id for item in evidence_ids)
            or any(
                occurrence_by_id[item]["occurrence_kind"]
                not in {*ANCHOR_KINDS, "repeat_or_alias_instruction"}
            ...
            raise ValueError("local repeat evidence drift")
        anchor_evidence_ids = [
        ...
        if resolution == "document_local_source_evidence_complete":
            if (
                ...
                raise ValueError("local resolved alias cardinality drift")
            classified_endpoint_ids = [*alias_ids, *canonical_ids]
            if not _endpoint_parents_resolve_equally(
                anchor_by_source_id,
                classified_endpoint_ids,
                ...
                raise ValueError("local resolved alias parent drift")
        elif resolution == COMPOSITE_CROSS_REFERENCE_RESOLUTION:
            same_span_context_ids = {
                item
                for item in anchor_evidence_ids
                if occurrence_by_id[item]["occurrence_kind"]
                == "context_anchor"
                and (
                    occurrence_by_id[item]["page_number"],
                    occurrence_by_id[item]["utf8_byte_start"],
                    occurrence_by_id[item]["utf8_byte_end"],
                )
                == (
                    source["page_number"],
            ...
            if (
                row["relation"] != "explicit_cross_reference"
                or source["occurrence_kind"] != "repeat_or_alias_instruction"
                or alias_ids
                or canonical_ids
                or len(same_span_context_ids) != 1
                or len(set(anchor_evidence_ids) - same_span_context_ids) < 2
                or any(
                    occurrence_by_id[item]["occurrence_kind"]
                    != "context_anchor"
                    for item in anchor_evidence_ids
                ...
                raise ValueError("local composite range evidence drift")
            classified_endpoint_ids = anchor_evidence_ids
        else:
            raise ValueError("local repeat resolution status drift")
        endpoint_classifications = {
            (
        ...
        if len(endpoint_classifications) != 1:
            raise ValueError("local repeat endpoint type drift")
        if row["relation"] == "same_printed_identifier_and_exact_label":
            compared = [
                anchor_by_source_id[item]
                for item in [*canonical_ids, *alias_ids]
            ]
            if (
                len(canonical_ids) != 1
                or len({item["exact_label"] for item in compared}) != 1
                or len({item["printed_identifier"] for item in compared}) != 1
                or compared[0]["printed_identifier"] is None
            ):
                raise ValueError("local repeat exact-label proof drift")
        repeat_fact_keys.add(fact_key)
        if resolution == "document_local_source_evidence_complete":
            _join_aliases(alias_parents, alias_ids[0], canonical_ids[0])
    ...
    instruction_covered = {
        ...
        if occurrence_by_id[source_id]["occurrence_kind"]
        == "repeat_or_alias_instruction"
    }
    if instruction_covered != expected:
        raise ValueError("local repeat exact cover drift")
...
def validate_annotation(
    ...
    _validate_local_repeat_laws(value)
...
def _mutations(value: Mapping[str, Any]) -> list[tuple[str, dict[str, Any]]]:
    ...
    def drop_remuneration_parent(row: dict[str, Any]) -> None:
        for anchor in row["local_anchor_classification_rows"]:
            if anchor["occurrence_kind"] == "remuneration_component_anchor":
                anchor["parent_source_occurrence_ids"] = []
                return
        raise ValueError("mutation fixture has no remuneration component")

    def assign_role_as_component_parent(row: dict[str, Any]) -> None:
        role_source_id = next(
        ...
        for anchor in row["local_anchor_classification_rows"]:
            if anchor["occurrence_kind"] == "remuneration_component_anchor":
                anchor["parent_source_occurrence_ids"] = [role_source_id]
                return
        raise ValueError("mutation fixture has no remuneration component")

    def composite_row(row: dict[str, Any]) -> dict[str, Any]:
        return next(
            repeat
            for repeat in row["local_repeat_alias_evidence_rows"]
            if repeat["resolution_status"]
            == COMPOSITE_CROSS_REFERENCE_RESOLUTION
        )

    def inject_composite_endpoint(row: dict[str, Any], key: str) -> None:
        repeat = composite_row(row)
        anchor_ids = {
            anchor["source_occurrence_id"]
            for anchor in row["local_anchor_classification_rows"]
        }
        endpoint = next(
            item
            for item in repeat["evidence_occurrence_ids"]
            if item in anchor_ids
        )
        repeat[key] = [endpoint]

    def omit_composite_target(row: dict[str, Any]) -> None:
        occurrence_by_id = {
            occurrence["questionnaire_occurrence_id"]: occurrence
            for occurrence in row["questionnaire_occurrence_rows"]
        }
        anchor_ids = {
            anchor["source_occurrence_id"]
        ...
        for repeat in row["local_repeat_alias_evidence_rows"]:
            if (
                repeat["resolution_status"]
                != COMPOSITE_CROSS_REFERENCE_RESOLUTION
            ):
                continue
            source = occurrence_by_id[repeat["source_occurrence_id"]]
            target_ids = [
                item
                for item in repeat["evidence_occurrence_ids"]
                if item in anchor_ids
            ...
            if len(target_ids) == 2:
                repeat["evidence_occurrence_ids"].remove(target_ids[0])
                return
        raise ValueError("mutation fixture has no two-target composite row")
    ...
    if value["local_repeat_alias_evidence_rows"]:
        ...
        add(
            "empty_alias_endpoint",
            lambda row: row["local_repeat_alias_evidence_rows"][0].__setitem__(
                "alias_anchor_source_occurrence_ids", []
            ),
        )
        add(
            "alias_endpoint_conflict",
            lambda row: row["local_repeat_alias_evidence_rows"][0].__setitem__(
                "canonical_anchor_source_occurrence_ids",
                row["local_repeat_alias_evidence_rows"][0][
                    "alias_anchor_source_occurrence_ids"
                ],
            ),
        )
        add(
            "omitted_repeat_evidence",
            lambda row: row["local_repeat_alias_evidence_rows"].pop(),
        )
        add(
            "composite_alias_endpoint",
            lambda row: inject_composite_endpoint(
                row, "alias_anchor_source_occurrence_ids"
            ),
        )
        add(
            "composite_canonical_endpoint",
            lambda row: inject_composite_endpoint(
                row, "canonical_anchor_source_occurrence_ids"
            ),
        )
        add("omitted_composite_target", omit_composite_target)
    add("orphan_remuneration_component", drop_remuneration_parent)
    add("wrong_component_parent_type", assign_role_as_component_parent)
    add(
        "printed_identifier_drift",
        lambda row: row["local_anchor_classification_rows"][0].__setitem__(
            "printed_identifier", "not-the-source-identifier"
        ),
    )
    add(
