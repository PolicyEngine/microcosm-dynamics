#!/usr/bin/env python3
"""Author the candidate-free source review for R_Q document 26.

All 59 physical pages of q80.pdf were reviewed against the authenticated
...
import build_rq_stage2_document_026_annotation as annotation
...
def author_review() -> dict[str, Any]:
    ...
    flow("d_route", 6, needle(6, "TURN TO P.14,"))
    flow("e_route", 6, needle(6, "TURN TO P. 18,", 0))
    flow("c_has_job", 6, needle(6, "GO TO C2 IF HEAD HAS JOB"))
    flow("e_otherwise", 6, block(6, "OTHERWISE", "SECTION E"))
    c_path = (("c_has_job",),)
    d_path = (("d_route",),)
...
def main() -> int:
    ...
    if args.check:
        if not annotation.REVIEW_PATH.exists():
            raise SystemExit("document 26 source review is missing")
        if annotation.REVIEW_PATH.read_bytes() != raw:
            raise SystemExit("document 26 source review is stale")
    else:
        annotation.REVIEW_PATH.parent.mkdir(parents=True, exist_ok=True)
        annotation.REVIEW_PATH.write_bytes(raw)
    print(
        f"document 26 source review: {len(value['page_review_rows'])} pages, "
