#!/usr/bin/env python3
"""Build and validate the stage-2 nonauthority annotation for document 64.
...
REVIEW_PATH = (
    ANNOTATION_ROOT / "document_064_fam2003_QxQs_source_review_v1.json"
)
OUTPUT_PATH = ANNOTATION_ROOT / "document_064_fam2003_QxQs_annotation_v1.json"
CAPTURE_ROOT = (
...
def _document_identity(
    ...
    if (
        ...
        raise ValueError("document 64 candidate-index resolution drift")
...
def _extract_page_texts(
    ...
    if questionnaire_inventory._pdftotext_version() != "26.04.0":
        raise ValueError("document 64 Poppler version drift")
    ...
    if len(page_texts) != 72 or len(page_texts) != len(expected):
        raise ValueError("document 64 page denominator drift")
    for page_text, page_row in zip(page_texts, expected, strict=True):
        ...
        if (
            ...
            raise ValueError("document 64 page replay drift")
    return page_texts
...
def _load_review(
    ...
    review = _strict_json(REVIEW_PATH, "document 64 source review")
...
def _load_candidates(
    ...
    if (
        ...
        raise ValueError("document 64 candidate raw identity drift")
    value = source_tools.strict_parse_document(raw, "document 64 candidates")
    if not isinstance(value, dict):
        raise ValueError("document 64 candidates are not an object")
    ...
    if (
        ...
        raise ValueError("document 64 candidate content identity drift")
    return value
...
def validate_review(
    ...
    if method != {
        ...
        "whole_page_review": "all_72_pages_including_empty_occurrence_pages",
...
def _validate_compatibility_predicate() -> None:
    ...
    root_row = {"interview_wave": 2003, "flow_branch_paths": [root]}
    left_or_right = {
        "interview_wave": 2003,
        "flow_branch_paths": [left, right],
    }
    left_row = {"interview_wave": 2003, "flow_branch_paths": [left]}
    right_row = {"interview_wave": 2003, "flow_branch_paths": [right]}
    other_wave = {"interview_wave": 2001, "flow_branch_paths": [left]}
    if (
...
def _mutations(value: Mapping[str, Any]) -> list[tuple[str, dict[str, Any]]]:
    ...
    def select_path_subset(row: dict[str, Any]) -> None:
        """Drop part of an occurrence's complete lawful resolving path set.

        A document that prints a multi-parent label loses one of its several
        applicable paths.  This document prints none, so the same law is
        exercised through its other real failure mode: a branch-scoped
        occurrence demoted to the bare root path, which is exactly what the
        stage-1 ``root_fallback_v1`` alternative would have selected.
        """
        ...
        for occurrence in row["questionnaire_occurrence_rows"]:
            if len(occurrence["flow_branch_paths"][0]) > 1:
                occurrence["flow_branch_paths"] = [[FLOW_ROOT]]
                return
        raise ValueError("mutation fixture has no branch-scoped occurrence")
