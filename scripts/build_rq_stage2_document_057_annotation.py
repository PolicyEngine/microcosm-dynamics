#!/usr/bin/env python3
"""Build and validate the stage-2 nonauthority annotation for document 57.
...
REVIEW_PATH = ANNOTATION_ROOT / "document_057_q96_source_review_v1.json"
OUTPUT_PATH = ANNOTATION_ROOT / "document_057_q96_annotation_v1.json"
CAPTURE_ROOT = (
...
def _document_identity(
    ...
    if (
        ...
        raise ValueError("document 57 candidate-index resolution drift")
...
def _extract_page_texts(
    ...
    if questionnaire_inventory._pdftotext_version() != "26.04.0":
        raise ValueError("document 57 Poppler version drift")
    ...
    if len(page_texts) != 141 or len(page_texts) != len(expected):
        raise ValueError("document 57 page denominator drift")
    for page_text, page_row in zip(page_texts, expected, strict=True):
        ...
        if (
            ...
            raise ValueError("document 57 page replay drift")
    return page_texts
...
def _load_review(
    ...
    review = _strict_json(REVIEW_PATH, "document 57 source review")
...
def _load_candidates(
    ...
    if (
        ...
        raise ValueError("document 57 candidate raw identity drift")
    value = source_tools.strict_parse_document(raw, "document 57 candidates")
    if not isinstance(value, dict):
        raise ValueError("document 57 candidates are not an object")
    ...
    if (
        ...
        raise ValueError("document 57 candidate content identity drift")
    return value
...
def validate_review(
    ...
    if method != {
        ...
        "whole_page_review": "all_141_pages_including_empty_occurrence_pages",
        ...
        for position, left in enumerate(same_kind_specs):
            for right in same_kind_specs[position + 1 :]:
                if right["utf8_byte_start"] >= left["utf8_byte_end"]:
                    break
                raise ValueError(
                    "document 57 source review has partially overlapping "
                    "same-kind atoms"
                )
    branch_specs = [
        spec
...
def _branch_compatible(
    ...
    *,
    ...
    # If a resolved witness extends one path from every occurrence, those
    # selected prefixes form a chain.  Its longest member is therefore one of
    # the occurrence paths.  Testing only those possible longest prefixes is
    # exactly equivalent to scanning the entire resolved domain for every
    # anchor, while avoiding an anchors x branches traversal on large forms.
    path_sets = [
...
def _validate_compatibility_predicate() -> None:
    ...
    root_row = {"interview_wave": 1996, "flow_branch_paths": [root]}
    left_or_right = {
        "interview_wave": 1996,
        "flow_branch_paths": [left, right],
    }
    left_row = {"interview_wave": 1996, "flow_branch_paths": [left]}
    right_row = {"interview_wave": 1996, "flow_branch_paths": [right]}
    other_wave = {"interview_wave": 1995, "flow_branch_paths": [left]}
    if (
