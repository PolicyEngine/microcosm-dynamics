#!/usr/bin/env python3
"""Build and validate the stage-2 nonauthority annotation for document 21.
...
REVIEW_PATH = (
    ANNOTATION_ROOT / "document_021_fam1978_QxQs_source_review_v1.json"
)
OUTPUT_PATH = ANNOTATION_ROOT / "document_021_fam1978_QxQs_annotation_v1.json"
CAPTURE_ROOT = (
...
def _document_identity(
    ...
    if (
        ...
        raise ValueError("document 21 candidate-index resolution drift")
...
def _extract_page_texts(
    ...
    if questionnaire_inventory._pdftotext_version() != "26.04.0":
        raise ValueError("document 21 Poppler version drift")
    ...
    if len(page_texts) != 95 or len(page_texts) != len(expected):
        raise ValueError("document 21 page denominator drift")
    for page_text, page_row in zip(page_texts, expected, strict=True):
        ...
        if (
            ...
            raise ValueError("document 21 page replay drift")
    return page_texts
...
def _load_review(
    ...
    review = _strict_json(REVIEW_PATH, "document 21 source review")
...
def _load_candidates(
    ...
    if (
        ...
        raise ValueError("document 21 candidate raw identity drift")
    value = source_tools.strict_parse_document(raw, "document 21 candidates")
    if not isinstance(value, dict):
        raise ValueError("document 21 candidates are not an object")
    ...
    if (
        ...
        raise ValueError("document 21 candidate content identity drift")
    return value
...
def validate_review(
    ...
    if method != {
        ...
        "whole_page_review": "all_95_pages_including_empty_occurrence_pages",
        ...
        for position, left in enumerate(same_kind_specs):
            for right in same_kind_specs[position + 1 :]:
                if right["utf8_byte_start"] >= left["utf8_byte_end"]:
                    break
                raise ValueError(
                    "document 21 source review has partially overlapping "
                    "same-kind atoms"
                )
    branch_specs = [
        spec
...
def _mutation_specs(value: Mapping[str, Any]) -> list[tuple[str, Any]]:
    ...
    def select_path_subset(row: dict[str, Any]) -> None:
        ...
        Every document-21 occurrence resolves under exactly one complete
    ...
    def collapse_reprint(row: dict[str, Any]) -> None:
        """Re-home a repeated screen's occurrence onto its first printing.

        The 1978 manual prints its extra-earner grid twice: printed page 32
        (document page 72) carries the first three earner columns and printed
        page 33 (document page 74) reprints the identical grid for three more,
        which its own objectives sheet describes as "a repetition of the
        previous one".  Silently folding the second printing back onto the
        first is the specific defect this document invites, so it is
        exercised directly.
        """

        for occurrence in row["questionnaire_occurrence_rows"]:
            if occurrence["page_number"] == 74:
                occurrence["page_number"] = 72
                return
        raise ValueError("mutation fixture has no repeated-screen row")
