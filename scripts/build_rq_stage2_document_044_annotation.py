#!/usr/bin/env python3
"""Build and validate the stage-2 nonauthority annotation for document 44.
...
REVIEW_PATH = ANNOTATION_ROOT / "document_044_q89_source_review_v1.json"
OUTPUT_PATH = ANNOTATION_ROOT / "document_044_q89_annotation_v1.json"
CAPTURE_ROOT = (
...
# The q89 image text layer has several stable OCR renderings of the printed
# HEAD token.  They are source evidence, not normalized output labels.  Keep
# the exact damaged bytes in each occurrence while classifying only these
# independently verified renderings as the head/reference-person role.
HEAD_ROLE_OCR_MARKERS = (
    "crud",
    "bud",
    "he&d",
    "hud",
...
def _document_identity(
    ...
    if (
        ...
        raise ValueError("document 44 candidate-index resolution drift")
...
def _extract_page_texts(
    ...
    if questionnaire_inventory._pdftotext_version() != "26.04.0":
        raise ValueError("document 44 Poppler version drift")
    ...
    if len(page_texts) != 61 or len(page_texts) != len(expected):
        raise ValueError("document 44 page denominator drift")
    for page_text, page_row in zip(page_texts, expected, strict=True):
        ...
        if (
            ...
            raise ValueError("document 44 page replay drift")
    return page_texts
...
def _load_review(
    ...
    review = _strict_json(REVIEW_PATH, "document 44 source review")
...
def _load_candidates(
    ...
    if (
        ...
        raise ValueError("document 44 candidate raw identity drift")
    value = source_tools.strict_parse_document(raw, "document 44 candidates")
    if not isinstance(value, dict):
        raise ValueError("document 44 candidates are not an object")
    ...
    if (
        ...
        raise ValueError("document 44 candidate content identity drift")
    return value
...
def _review_role_classification(matched_text: str) -> str:
    classification = stage1_candidates._role_classification(matched_text)
    lowered = matched_text.casefold()
    if classification == "head_or_reference_person" or any(
        marker in lowered for marker in HEAD_ROLE_OCR_MARKERS
    ):
...
def validate_review(
    ...
    if method != {
        ...
        "whole_page_review": "all_61_pages_including_empty_occurrence_pages",
    ...
    # and one flow atom may still apply on several complete parent paths.
        ...
        for position, left in enumerate(same_kind_specs):
            for right in same_kind_specs[position + 1 :]:
                if right["utf8_byte_start"] >= left["utf8_byte_end"]:
                    break
                raise ValueError(
                    "document 44 source review has partially overlapping "
                    "same-kind atoms"
                )
    branch_specs = [
        spec
...
def _branch_compatible(
    ...
    *,
    resolved_path_index: Sequence[tuple[str, ...]] | None = None,
) -> bool:
    ...
    if (
        ...
        return False
    path_sets = [
        {tuple(path) for path in occurrence["flow_branch_paths"]}
        for occurrence in occurrences
    ]
    path_lengths = [
        sorted({len(path) for path in paths}) for paths in path_sets
    ]
    candidates = sorted(
        set().union(*path_sets), key=lambda path: (-len(path), path)
    )
    indexed_paths = (
        sorted(tuple(path) for path in resolved_paths)
        if resolved_path_index is None
        else resolved_path_index
    )
    for candidate in candidates:
        if not all(
            any(
                candidate[:length] in paths
                for length in lengths
                if length <= len(candidate)
            )
            for paths, lengths in zip(path_sets, path_lengths, strict=True)
        ):
            continue
        position = bisect_left(indexed_paths, candidate)
        if position < len(indexed_paths) and _path_is_prefix(
            candidate, indexed_paths[position]
        ):
            return True
    return False
...
def _validate_compatibility_predicate() -> None:
    ...
    root_row = {"interview_wave": 1991, "flow_branch_paths": [root]}
    left_or_right = {
        "interview_wave": 1991,
        "flow_branch_paths": [left, right],
    }
    left_row = {"interview_wave": 1991, "flow_branch_paths": [left]}
    right_row = {"interview_wave": 1991, "flow_branch_paths": [right]}
    other_wave = {"interview_wave": 1987, "flow_branch_paths": [left]}
    if (
...
def _validate_local_anchor_laws(value: Mapping[str, Any]) -> None:
    ...
    resolved_path_index = sorted(tuple(path) for path in resolved_paths)
    component_kinds = {
        "remuneration_component_anchor",
        "context_anchor",
    }
