#!/usr/bin/env python3
"""Build and validate the stage-2 nonauthority annotation for document 59.
...
REVIEW_PATH = ANNOTATION_ROOT / "document_059_q97_source_review_v1.json"
OUTPUT_PATH = ANNOTATION_ROOT / "document_059_q97_annotation_v1.json"
CAPTURE_ROOT = (
...
# These pages are wholly inside a source-established conditional section or
# subroute.  A root path on a non-flow fact there would silently discard the
# controlling questionnaire branch.  Mixed entry/checkpoint pages (11, 46,
# 81, 92, 97, 141, 153, and 166) are checked by their exact review paths
# instead of this page-level guard.  Pages 82--84 also contain true printed
# G-section reconvergences (G12, G18a--c, and G22), so their conditional
# subranges are checked by the exact controller audit rather than treated as
# wholly conditional pages.
CONDITIONAL_NONFLOW_PAGES = frozenset(
    {
        *range(12, 46),
        *range(47, 78),
        98,
...
def _review_role_classification(matched_text: str) -> str:
    """Classify exact q97 role lexemes, including reviewed OCR spellings."""

    normalized = " ".join(matched_text.split()).casefold()
    if normalized in {
        "bead",
        "brad",
        "he.ad",
    ...
    return stage1_candidates._role_classification(matched_text)
...
def _document_identity(
    ...
    if (
        ...
        raise ValueError("document 59 candidate-index resolution drift")
...
def _extract_page_texts(
    ...
    if questionnaire_inventory._pdftotext_version() != "26.04.0":
        raise ValueError("document 59 Poppler version drift")
    ...
    if len(page_texts) != 187 or len(page_texts) != len(expected):
        raise ValueError("document 59 page denominator drift")
    for page_text, page_row in zip(page_texts, expected, strict=True):
        ...
        if (
            ...
            raise ValueError("document 59 page replay drift")
    return page_texts
...
def _load_review(
    ...
    review = _strict_json(REVIEW_PATH, "document 59 source review")
...
def _load_candidates(
    ...
    if (
        ...
        raise ValueError("document 59 candidate raw identity drift")
    value = source_tools.strict_parse_document(raw, "document 59 candidates")
    if not isinstance(value, dict):
        raise ValueError("document 59 candidates are not an object")
    ...
    if (
        ...
        raise ValueError("document 59 candidate content identity drift")
    return value
...
def validate_review(
    ...
    if method != {
        ...
        "whole_page_review": "all_187_pages_including_empty_occurrence_pages",
    ...
    for spec in occurrence_specs:
        ...
        if spec["occurrence_kind"] == "flow_branch_label" and not any(
            character.isalnum() for character in matched_text
        ):
            raise ValueError(
                "source review flow label lacks a semantic alphanumeric token"
            )
        ...
        if (
            spec["occurrence_kind"] != "flow_branch_label"
            and page_number in CONDITIONAL_NONFLOW_PAGES
            and [] in paths
        ):
            raise ValueError(
                "document 59 conditional non-flow occurrence has root path"
            )
        ...
        last_order = order
    # The independent document-59 source ledger selects one semantic atom at
        ...
        for position, left in enumerate(same_kind_specs):
            for right in same_kind_specs[position + 1 :]:
                if right["utf8_byte_start"] >= left["utf8_byte_end"]:
                    break
                raise ValueError(
                    "document 59 source review has partially overlapping "
                    "same-kind atoms"
                )
    branch_specs = [
        spec
        ...
        if occurrence is not None:
            ...
            if kind == "role_anchor":
                ...
                expected_classification = (
                    "role",
                    _review_role_classification(matched_text),
                )
            else:
...
def _validate_compatibility_predicate() -> None:
    ...
    root_row = {"interview_wave": 1997, "flow_branch_paths": [root]}
    left_or_right = {
        "interview_wave": 1997,
        "flow_branch_paths": [left, right],
    }
    left_row = {"interview_wave": 1997, "flow_branch_paths": [left]}
    right_row = {"interview_wave": 1997, "flow_branch_paths": [right]}
...
def run_mutation_tests(
    ...
    root_path_mutation = copy.deepcopy(inputs[5])
    conditional_spec = next(
        spec
        for spec in root_path_mutation["occurrence_specs"]
        if spec["occurrence_kind"] != "flow_branch_label"
        and spec["page_number"] in CONDITIONAL_NONFLOW_PAGES
        and [] not in spec["parent_review_branch_paths"]
    )
    conditional_spec["parent_review_branch_paths"] = [[]]
    root_path_mutation["integrity"]["content_sha256"] = _content_sha256(
        root_path_mutation
    )
    try:
        validate_review(root_path_mutation, document, page_texts)
    except ValueError as error:
        if "conditional non-flow occurrence has root path" not in str(error):
            raise ValueError(
                "conditional-root mutation failed for the wrong reason"
            ) from error
    else:
        raise ValueError(
            "mutation was not rejected: conditional_nonflow_root_path"
        )

    unlabeled_flow_mutation = copy.deepcopy(inputs[5])
    flow_spec = next(
        spec
        for spec in unlabeled_flow_mutation["occurrence_specs"]
        if spec["occurrence_kind"] == "flow_branch_label"
    )
    flow_intervals_by_page: dict[int, list[tuple[int, int]]] = defaultdict(
        list
    )
    for spec in unlabeled_flow_mutation["occurrence_specs"]:
        if (
            spec["occurrence_kind"] == "flow_branch_label"
            and spec is not flow_spec
        ):
            flow_intervals_by_page[spec["page_number"]].append(
                (spec["utf8_byte_start"], spec["utf8_byte_end"])
            )
    whitespace_coordinate: tuple[int, int] | None = None
    for page_number, page_text in enumerate(page_texts, start=1):
        for start, byte in enumerate(page_text.encode("utf-8")):
            if byte not in b" \t\r\n" or any(
                left <= start < right
                for left, right in flow_intervals_by_page[page_number]
            ):
                continue
            whitespace_coordinate = (page_number, start)
            break
        if whitespace_coordinate is not None:
            break
    if whitespace_coordinate is None:
        raise ValueError("unlabeled-flow mutation fixture is unavailable")
    page_number, start = whitespace_coordinate
    flow_spec["page_number"] = page_number
    flow_spec["utf8_byte_start"] = start
    flow_spec["utf8_byte_end"] = start + 1
    flow_spec["parent_review_branch_paths"] = [[]]
    flow_spec["review_occurrence_id"] = (
        "rq-review-occurrence:"
        + _canonical_digest(
            [
                document["source_document_id"],
    ...
    unlabeled_flow_mutation["occurrence_specs"].sort(
        key=lambda row: (
            row["page_number"],
            row["utf8_byte_start"],
            row["utf8_byte_end"],
    ...
    unlabeled_flow_mutation["integrity"]["content_sha256"] = _content_sha256(
        unlabeled_flow_mutation
    )
    try:
        validate_review(unlabeled_flow_mutation, document, page_texts)
    except ValueError as error:
        if "flow label lacks a semantic alphanumeric token" not in str(error):
            raise ValueError(
                "unlabeled-flow mutation failed for the wrong reason"
            ) from error
    else:
        raise ValueError("mutation was not rejected: unlabeled_flow")
