#!/usr/bin/env python3
"""Build and validate the stage-2 nonauthority annotation for document 29.
...
REVIEW_PATH = (
    ANNOTATION_ROOT / "document_029_fam1982_QxQs_source_review_v1.json"
)
OUTPUT_PATH = ANNOTATION_ROOT / "document_029_fam1982_QxQs_annotation_v1.json"
CAPTURE_ROOT = (
...
def _document_identity(
    ...
    if (
        ...
        raise ValueError("document 29 candidate-index resolution drift")
...
def _extract_page_texts(
    ...
    if questionnaire_inventory._pdftotext_version() != "26.04.0":
        raise ValueError("document 29 Poppler version drift")
    ...
    if len(page_texts) != 125 or len(page_texts) != len(expected):
        raise ValueError("document 29 page denominator drift")
    for page_text, page_row in zip(page_texts, expected, strict=True):
        ...
        if (
            ...
            raise ValueError("document 29 page replay drift")
    return page_texts
...
def _load_review(
    ...
    review = _strict_json(REVIEW_PATH, "document 29 source review")
...
def _load_candidates(
    ...
    if (
        ...
        raise ValueError("document 29 candidate raw identity drift")
    value = source_tools.strict_parse_document(raw, "document 29 candidates")
    if not isinstance(value, dict):
        raise ValueError("document 29 candidates are not an object")
    ...
    if (
        ...
        raise ValueError("document 29 candidate content identity drift")
    return value
...
def validate_review(
    ...
    if method != {
        ...
        "whole_page_review": "all_125_pages_including_empty_occurrence_pages",
...
def _match_candidate_occurrences(
    ...
    The relation is deliberately many-to-many.  A reviewer may merge several
    machine fragments into one source atom, or split one over-broad candidate
    into several atoms.  Every same-kind overlap therefore belongs in both
    projections; an exact-coordinate match must not hide additional overlaps.
    """
    ...
    for candidate in candidate_rows:
        ...
        exact_coordinate = (
            len(overlaps) == 1
            and overlaps[0]["utf8_byte_start"] == candidate["utf8_byte_start"]
            and overlaps[0]["utf8_byte_end"] == candidate["utf8_byte_end"]
        )
        if len(output_ids) > 1:
            disposition = "split"
        elif exact_coordinate:
...
def _adjudicate(
    ...
    for candidate in candidates["candidate_occurrence_rows"]:
        ...
        if disposition == "rejected":
            ...
            corrected_spans = ", ".join(
                (
                    f"{occurrence_by_id[output_id]['utf8_byte_start']}:"
                    f"{occurrence_by_id[output_id]['utf8_byte_end']} "
                    f"({occurrence_by_id[output_id]['occurrence_kind']})"
                )
                for output_id in output_ids
            )
            notes.append(
    ...
    for (
        ...
        exactly_claimed_branch_ids: set[str] = set()
        # Retain every candidate whose complete source ancestry translates
        # exactly.  Several machine fragments can merge into one reviewed
        # label, and more than one candidate may therefore project to the same
        # branch.  A single candidate path may likewise split into several
        # semantic branch rows when the printed label applies on several
        # complete paths.
        for candidate in flow_candidates:
            ...
            matching = [
                ...
                if branch["branch_path"][:-1] in translated_parent_paths
            ]
            ...
            for branch_id in matching:
                ...
                exactly_claimed_branch_ids.add(branch_id)

        # A root-fallback candidate is source evidence for any retained labels
        # left after exact ancestry matching for this candidate occurrence, but
        # its path must be corrected.
        fallback = next(
            (
                row
        ...
        if fallback is not None:
            ...
            for branch_id in [
                branch["flow_branch_id"]
                for branch in output_branches
                if branch["flow_branch_id"] not in exactly_claimed_branch_ids
            ]:
                flow_candidate_to_branches[candidate_id].append(branch_id)
                flow_branch_to_candidates[branch_id].append(candidate_id)
    ...
    for branch in branches:
        ...
        if not candidate_ids:
            ...
            if "split" in dispositions_for_output:
                action = "candidate_split"
            elif dispositions_for_output == {"accepted"}:
                action = "candidate_accepted"
            else:
                action = "candidate_modified"
        output_adjudications.append(
...
def _validate_compatibility_predicate() -> None:
    ...
    root_row = {"interview_wave": 1982, "flow_branch_paths": [root]}
    left_or_right = {
        "interview_wave": 1982,
        "flow_branch_paths": [left, right],
    }
    left_row = {"interview_wave": 1982, "flow_branch_paths": [left]}
    right_row = {"interview_wave": 1982, "flow_branch_paths": [right]}
