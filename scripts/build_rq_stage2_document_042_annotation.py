#!/usr/bin/env python3
"""Build and validate the stage-2 nonauthority annotation for document 42.
...
REVIEW_PATH = ANNOTATION_ROOT / "document_042_q88_source_review_v1.json"
OUTPUT_PATH = ANNOTATION_ROOT / "document_042_q88_annotation_v1.json"
CAPTURE_ROOT = (
...
def _document_identity(
    ...
    if (
        ...
        raise ValueError("document 42 candidate-index resolution drift")
...
def _extract_page_texts(
    ...
    if questionnaire_inventory._pdftotext_version() != "26.04.0":
        raise ValueError("document 42 Poppler version drift")
    ...
    if len(page_texts) != 64 or len(page_texts) != len(expected):
        raise ValueError("document 42 page denominator drift")
    for page_text, page_row in zip(page_texts, expected, strict=True):
        ...
        if (
            ...
            raise ValueError("document 42 page replay drift")
    return page_texts
...
def _load_review(
    ...
    review = _strict_json(REVIEW_PATH, "document 42 source review")
...
def _load_candidates(
    ...
    if (
        ...
        raise ValueError("document 42 candidate raw identity drift")
    value = source_tools.strict_parse_document(raw, "document 42 candidates")
    if not isinstance(value, dict):
        raise ValueError("document 42 candidates are not an object")
    ...
    if (
        ...
        raise ValueError("document 42 candidate content identity drift")
    return value
...
def _validate_document_42_semantics(
    occurrence_specs: Sequence[Mapping[str, Any]],
    branch_ref_to_spec: Mapping[str, tuple[Mapping[str, Any], int]],
) -> None:
    """Reject structurally valid regressions in the reviewed q88 routing."""

    def coordinate(spec: Mapping[str, Any]) -> tuple[int, int, int]:
        return (
            spec["page_number"],
    ...
    def one(
        ...
        matches = [
            ...
            if coordinate(spec) == (page_number, start, end)
            and spec["occurrence_kind"] == occurrence_kind
        ]
        if len(matches) != 1:
            raise ValueError(
                "document 42 semantic occurrence domain drift at "
                f"{(page_number, start, end, occurrence_kind)}"
            )
        return matches[0]
    ...
    def absent(
        ...
        if any(
            coordinate(spec) == (page_number, start, end)
            and spec["occurrence_kind"] == occurrence_kind
            for spec in occurrence_specs
        ):
            raise ValueError(
                "document 42 rejected semantic atom reintroduced at "
                f"{(page_number, start, end, occurrence_kind)}"
            )
    ...
    def paths(
        ...
        return {
            tuple(
                coordinate(branch_ref_to_spec[branch_ref][0])
    ...
    def expect_paths(
        ...
        occurrence_kind: str,
        expected: set[tuple[tuple[int, int, int], ...]],
    ) -> None:
        if paths(one(page_number, start, end, occurrence_kind)) != expected:
            raise ValueError(
                "document 42 semantic path drift at "
                f"{(page_number, start, end, occurrence_kind)}"
            )

    # Page 4: the direct-work and B3-YES routes converge at B4.  The B7
    # shortcut is retained as an exact terminal branch but collapses at B9,
    # where its complementary YES bytes are absent.
    h13 = (4, 1602, 1684)
    h1 = (4, 1890, 1904)
    h2 = (4, 1915, 1949)
    h3 = (4, 1968, 1982)
    b_next_12 = (4, 2393, 2408)
    h48 = (4, 2426, 2491)
    h4 = (4, 2514, 2524)
    b3_yes = (4, 3721, 3727)
    b3_next = (4, 4472, 4481)
    b3_parents = {(h13, h3), (h48,)}
    b_main = {
        (h13, h1, b_next_12),
        (h13, h2, b_next_12),
        (h13, h3, b3_yes, b3_next),
        (h48, b3_yes, b3_next),
    }
    expect_paths(4, 3721, 3727, "flow_branch_label", b3_parents)
    expect_paths(4, 4489, 4496, "flow_branch_label", b3_parents)
    expect_paths(
        4,
        4472,
        4481,
        "flow_branch_label",
        {(h13, h3, b3_yes), (h48, b3_yes)},
    )
    expect_paths(
        4,
        3773,
    ...
    for occurrence_kind in ("context_anchor", "field_purpose_prompt"):
        expect_paths(4, 413, 567, occurrence_kind, b_main)
        expect_paths(4, 5463, 5584, occurrence_kind, b_main)
    for start, end, expected in (
        ...
        expect_paths(4, start, end, "context_anchor", expected)
    expect_paths(4, 2345, 2355, "context_anchor", {(h13, h3)})
    expect_paths(4, 3868, 3886, "context_anchor", b3_parents)
    absent(4, 1890, 1982, "context_anchor")
    absent(4, 2514, 2562, "context_anchor")

    # C6 YES alone reaches C7/C8; its NO route exits to Section D.  The C8
    # terminal instruction and the role atom inside C8 share that same screen.
    c6_yes = (11, 2683, 2689)
    c6_yes_paths = {
        (*path, c6_yes)
        for path in paths(one(11, 2683, 2689, "flow_branch_label"))
    }
    expect_paths(11, 3148, 3260, "field_purpose_prompt", c6_yes_paths)
    expect_paths(11, 3604, 3773, "field_purpose_prompt", c6_yes_paths)
    expect_paths(11, 3729, 3733, "role_anchor", c6_yes_paths)
    expect_paths(11, 4109, 4158, "flow_branch_label", c6_yes_paths)

    # Page 16: only status 3 and statuses 4-8 reach D3 and therefore Section
    # E.  Codes 1/2 reach D4 directly.  The missing D3 answer bytes are never
    # represented by an invented branch.
    d_male = (16, 1152, 1164)
    d_female = (16, 1165, 1182)
    wife_present = (16, 1828, 1859)
    wife_absent = (16, 1860, 1891)
    w13 = (16, 2300, 2385)
    w1 = (16, 2628, 2642)
    w2 = (16, 2657, 2691)
    w3 = (16, 2708, 2722)
    ...
    w7 = (16, 5111, 5121)
    w8 = (16, 5144, 5162)
    wife_base = (d_male, wife_present)
    wife_main = {(*wife_base, w13, status) for status in (w1, w2, w3)} | {
        (*wife_base, w48, status) for status in (w4, w5, w6, w7, w8)
    }
    d3_parents = {
        (*wife_base, w13, w3),
        *{(*wife_base, w48, status) for status in (w4, w5, w6, w7, w8)},
    ...
    for occurrence_kind in ("context_anchor", "field_purpose_prompt"):
        expect_paths(16, 388, 488, occurrence_kind, wife_main)
    for start, end, expected in (
        (1152, 1156, {(d_male,)}),
        (1168, 1172, {(d_female,)}),
        (1831, 1842, {(d_male, wife_present)}),
        (1866, 1877, {(d_male, wife_absent)}),
        (2130, 2143, {wife_base}),
        (3585, 3598, {(*wife_base, w48, w4)}),
    ):
        expect_paths(16, start, end, "role_anchor", expected)
    for occurrence_kind in ("context_anchor", "field_purpose_prompt"):
        expect_paths(16, 2073, 2185, occurrence_kind, {wife_base})
    expect_paths(16, 2191, 2284, "context_anchor", {wife_base})
    for start, end, expected in (
        (2628, 2642, {(*wife_base, w13, w1)}),
        (2657, 2691, {(*wife_base, w13, w2)}),
        (2708, 2722, {(*wife_base, w13, w3)}),
        (3470, 3480, {(*wife_base, w48, w4)}),
    ):
        expect_paths(16, start, end, "context_anchor", expected)
    expect_paths(
        16,
        3501,
        3526,
    ...
    expect_paths(16, 3906, 3909, "field_purpose_prompt", d3_parents)
    expect_paths(16, 4003, 4017, "context_anchor", d3_parents)
    expect_paths(16, 4033, 4164, "context_anchor", wife_main)
    absent(16, 2628, 2722, "context_anchor")
    absent(16, 4003, 4164, "context_anchor")

    # Section E: E4 is the E1 merge, E5 and the employment-history pages use
    # E4 YES, and E6-E8 use E4 YES/NO because the older-year E5 labels are
    # absent from the exact page bytes.
    e_entry = (22, 207, 308)
    e4_yes = (23, 776, 782)
    e4_no = (23, 812, 837)
    e_entry_paths = {(*path, e_entry) for path in d3_parents}
    e4_yes_paths = {(*path, e4_yes) for path in e_entry_paths}
    e4_no_paths = {(*path, e4_no) for path in e_entry_paths}
    expect_paths(22, 207, 308, "flow_branch_label", d3_parents)
    expect_paths(22, 595, 628, "flow_branch_label", e_entry_paths)
    expect_paths(23, 776, 782, "flow_branch_label", e_entry_paths)
    expect_paths(23, 812, 837, "flow_branch_label", e_entry_paths)
    expect_paths(23, 569, 742, "field_purpose_prompt", e_entry_paths)
    expect_paths(23, 854, 987, "field_purpose_prompt", e4_yes_paths)
    expect_paths(
        23,
        2092,
        2200,
        "field_purpose_prompt",
        e4_yes_paths | e4_no_paths,
    )
    expect_paths(
        23,
        3316,
    ...
    absent(26, 6255, 6267, "flow_branch_label")

    # The page-27 E74/E86 columns are interleaved in source order.  Their two
    # local loops stay separate; E98 is the E96 merge before ASK E87-E98.
    e86_yes = (27, 1055, 1058)
    e86_no = (27, 1088, 1134)
    e74_yes = (27, 1154, 1160)
    e74_no = (27, 1190, 1259)
    e96_yes = (27, 9778, 9784)
    e96_no = (27, 9799, 9837)
    e84_yes = (27, 10363, 10369)
    e84_no = (27, 10380, 10425)
    e98_yes = (27, 12023, 12026)
    e98_no = (27, 12056, 12100)
    e_repeat = (27, 12521, 12552)
    for coordinate_value in (e86_yes, e86_no, e74_yes, e74_no):
        expect_paths(
            *coordinate_value,
            "flow_branch_label",
            e4_yes_paths,
        )
    e86_yes_paths = {(*path, e86_yes) for path in e4_yes_paths}
    e74_yes_paths = {(*path, e74_yes) for path in e4_yes_paths}
    for coordinate_value in (e96_yes, e96_no):
        expect_paths(
            *coordinate_value,
            "flow_branch_label",
            e86_yes_paths,
        )
    for coordinate_value in (e84_yes, e84_no):
        expect_paths(
            *coordinate_value,
            "flow_branch_label",
            e74_yes_paths,
    ...
    expect_paths(
        ...
        10333,
        "field_purpose_prompt",
        {(*path, e96_yes) for path in e86_yes_paths},
    )
    expect_paths(
        27,
        11324,
        11393,
        "field_purpose_prompt",
        {(*path, e84_yes) for path in e74_yes_paths},
    )
    expect_paths(27, 11706, 11800, "field_purpose_prompt", e86_yes_paths)
    for coordinate_value in (e98_yes, e98_no):
        expect_paths(
            *coordinate_value,
            "flow_branch_label",
            e86_yes_paths,
        )
    e98_yes_paths = {(*path, e98_yes) for path in e86_yes_paths}
    expect_paths(*e_repeat, "flow_branch_label", e98_yes_paths)
    expect_paths(*e_repeat, "repeat_or_alias_instruction", e98_yes_paths)

    # Candidate false positives and compound-role granularity are sealed as
    # document-specific negative/positive evidence.
    for page_number, start, end in (
        (5, 497, 512),
        (5, 1207, 1221),
        (17, 503, 520),
        ...
        absent(page_number, start, end, "job_anchor")
    for page_number, start, end in (
        (59, 209, 220),
        (59, 1311, 1322),
        (61, 8169, 8182),
    ):
        one(page_number, start, end, "role_anchor")
    for page_number, start, end in (
        (59, 209, 213),
        (59, 214, 220),
        (59, 1311, 1315),
        ...
        absent(page_number, start, end, "role_anchor")
    ...
    for page_number, start, end in (
        ...
        flow = one(page_number, start, end, "flow_branch_label")
        repeat = one(page_number, start, end, "repeat_or_alias_instruction")
        if paths(flow) != paths(repeat):
            raise ValueError("document 42 ASK-again ancestry drift")

    # B106/C98/D98 are merge screens.  Match the sealed q92 convention: a
    # merge returns to the common resolving parent, rather than selecting one
    # alternative or serializing a redundant prefix-plus-extension union.
    b106_screen = paths(one(9, 11809, 11871, "flow_branch_label"))
    expect_paths(9, 12075, 12103, "flow_branch_label", b106_screen)
    for (
        page_number,
        yes_start,
        yes_end,
        ...
        (15, 13356, 13359, 13385, 13455, 13848, 13876),
        (21, 9073, 9076, 9108, 9171, 9361, 9389),
    ):
        merge_screen = paths(
            one(page_number, no_start, no_end, "flow_branch_label")
        )
        expect_paths(
            page_number,
            yes_start,
        ...
        expect_paths(
            ...
            "flow_branch_label",
            {
                (*path, (page_number, yes_start, yes_end))
                for path in merge_screen
            },
        )

    # An exact anchor wholly contained by one purpose prompt is evidence on
    # that same conditional screen.  Partial overlaps are deliberately not
    # inferred (the page-4 and page-16 cross-column composites were split).
    purposes_by_page: dict[int, list[Mapping[str, Any]]] = defaultdict(list)
    for spec in occurrence_specs:
        if spec["occurrence_kind"] == "field_purpose_prompt":
            purposes_by_page[spec["page_number"]].append(spec)
    for spec in occurrence_specs:
        if spec["occurrence_kind"] not in ANCHOR_KINDS:
            continue
        containers = [
            purpose
            for purpose in purposes_by_page[spec["page_number"]]
            if purpose["utf8_byte_start"] <= spec["utf8_byte_start"]
            and spec["utf8_byte_end"] <= purpose["utf8_byte_end"]
        ]
        if len(containers) == 1 and paths(spec) != paths(containers[0]):
            raise ValueError("document 42 contained anchor/purpose path drift")
...
def validate_review(
    ...
    for spec in occurrence_specs:
        ...
        last_order = order
    # The independent document-42 source ledger selects one semantic atom at
        ...
        for position, left in enumerate(same_kind_specs):
            for right in same_kind_specs[position + 1 :]:
                if right["utf8_byte_start"] >= left["utf8_byte_end"]:
                    break
                raise ValueError(
                    "document 42 source review has partially overlapping "
                    "same-kind atoms"
                )
    branch_specs = [
        spec
    ...
    _validate_document_42_semantics(occurrence_specs, branch_ref_to_spec)
...
def run_mutation_tests(
    ...
    for source_spec in review_mutation["occurrence_specs"]:
        is_root_flow_label = source_spec[
            "occurrence_kind"
        ] == "flow_branch_label" and source_spec[
            "parent_review_branch_paths"
        ] == [
            []
        ]
