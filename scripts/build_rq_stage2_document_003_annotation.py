#!/usr/bin/env python3
"""Build and validate the stage-2 nonauthority annotation for document 3.
...
REVIEW_PATH = (
    ANNOTATION_ROOT / "document_003_fam1969_QxQs_source_review_v1.json"
)
OUTPUT_PATH = ANNOTATION_ROOT / "document_003_fam1969_QxQs_annotation_v1.json"
CAPTURE_ROOT = (
...
def _document_identity(
    ...
    if (
        ...
        raise ValueError("document 3 candidate-index resolution drift")
...
def _extract_page_texts(
    ...
    if len(page_texts) != 47 or len(page_texts) != len(expected):
        raise ValueError("document 3 page denominator drift")
    for page_text, page_row in zip(page_texts, expected, strict=True):
        ...
        if (
            ...
            raise ValueError("document 3 page replay drift")
    return page_texts
...
def _load_review(
    ...
    review = _strict_json(REVIEW_PATH, "document 3 source review")
...
def _load_candidates(
    ...
    if (
        ...
        raise ValueError("document 3 candidate raw identity drift")
    value = source_tools.strict_parse_document(raw, "document 3 candidates")
    if not isinstance(value, dict):
        raise ValueError("document 3 candidates are not an object")
    ...
    if (
        ...
        raise ValueError("document 3 candidate content identity drift")
    return value
...
def _source_printed_identifier(page_text: str, byte_start: int) -> str | None:
    ...
    for index, row in enumerate(rows):
        line_byte_start = len(page_text[: row["start"]].encode("utf-8"))
        line_byte_end = len(page_text[: row["end"]].encode("utf-8"))
...
def validate_review(
    ...
    if method != {
        ...
        "whole_page_review": "all_47_pages_including_empty_occurrence_pages",
    ...
    for spec in occurrence_specs:
        ...
        last_order = order
    # The independent document-13 source ledger selects one semantic atom at
        ...
        for position, left in enumerate(same_kind_specs):
            for right in same_kind_specs[position + 1 :]:
                if right["utf8_byte_start"] >= left["utf8_byte_end"]:
                    break
                raise ValueError(
                    "document 3 source review has partially overlapping "
                    "same-kind atoms"
                )
    branch_specs = [
        spec
    ...
    evidence_source_covered: set[str] = set()
    for row in repeat_specs:
        ...
        evidence_source_covered.add(row["review_occurrence_id"])
...
def _adjudicate(
    ...
    for candidate in candidates["candidate_anchor_classification_rows"]:
        ...
        if not output_rows:
            ...
            parent_candidate_ids = candidate["parent_anchor_candidate_ids"]
            every_parent_resolved = all(
                candidate_occurrence_outputs.get(parent_candidate_id)
                for parent_candidate_id in parent_candidate_ids
            )
            for parent_candidate_id in parent_candidate_ids:
...
def _validate_occurrence_locator_laws(
    value: Mapping[str, Any], document: Mapping[str, Any]
) -> None:
    """Recompute every locator digest and enforce its unique coordinate."""
    ...
    for row in rows:
        expected_locator_digest = _canonical_digest(
            [
        ...
        expected_occurrence_id = (
            ...
            + _canonical_digest([row[key] for key in OCCURRENCE_KEYS[1:]])
        )
        coordinate = (
            row["source_document_id"],
            row["page_number"],
        ...
        if (
            row["source_locator_sha256"] != expected_locator_digest
            or row["questionnaire_occurrence_id"] != expected_occurrence_id
            or expected_locator_digest in locator_digests
            or expected_occurrence_id in occurrence_ids
            or coordinate in coordinates
        ):
            raise ValueError("occurrence locator, ID, or coordinate drift")
...
def validate_annotation(
    ...
    _validate_occurrence_locator_laws(value, document)
