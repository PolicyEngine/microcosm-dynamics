#!/usr/bin/env python3
"""Build and validate the stage-2 nonauthority annotation for document 13.
...
REVIEW_PATH = (
    ANNOTATION_ROOT / "document_013_fam1974_QxQs_source_review_v1.json"
)
OUTPUT_PATH = ANNOTATION_ROOT / "document_013_fam1974_QxQs_annotation_v1.json"
CAPTURE_ROOT = (
...
def _document_identity(
    ...
    if (
        ...
        raise ValueError("document 13 candidate-index resolution drift")
...
def _extract_page_texts(
    ...
    if questionnaire_inventory._pdftotext_version() != "26.04.0":
        raise ValueError("document 13 Poppler version drift")
    ...
    if len(page_texts) != 62 or len(page_texts) != len(expected):
        raise ValueError("document 13 page denominator drift")
    for page_text, page_row in zip(page_texts, expected, strict=True):
        ...
        if (
            ...
            raise ValueError("document 13 page replay drift")
    return page_texts
...
def _load_review(
    ...
    review = _strict_json(REVIEW_PATH, "document 13 source review")
...
def _load_candidates(
    ...
    if (
        ...
        raise ValueError("document 13 candidate raw identity drift")
    value = source_tools.strict_parse_document(raw, "document 13 candidates")
    if not isinstance(value, dict):
        raise ValueError("document 13 candidates are not an object")
    ...
    if (
        ...
        raise ValueError("document 13 candidate content identity drift")
    return value
...
def _source_printed_identifier(page_text: str, byte_start: int) -> str | None:
    for line in stage1_candidates._physical_lines(page_text):
        ...
        if line_byte_start <= byte_start < line_byte_end:
            ...
            # The 1974 OCR commonly inserts whitespace before the period, so
        ...
        for position, left in enumerate(same_kind_specs):
            for right in same_kind_specs[position + 1 :]:
                if right["utf8_byte_start"] >= left["utf8_byte_end"]:
                    break
                raise ValueError(
                    "document 13 source review has partially overlapping "
                    "same-kind atoms"
                )
    branch_specs = [
        spec
