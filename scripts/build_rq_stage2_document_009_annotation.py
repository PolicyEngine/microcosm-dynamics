#!/usr/bin/env python3
"""Build and validate the stage-2 nonauthority annotation for document 9.
...
REVIEW_PATH = (
    ANNOTATION_ROOT / "document_009_fam1972_QxQs_source_review_v1.json"
)
OUTPUT_PATH = ANNOTATION_ROOT / "document_009_fam1972_QxQs_annotation_v1.json"
CAPTURE_ROOT = (
...
def _document_identity(
    ...
    if (
        ...
        raise ValueError("document 9 candidate-index resolution drift")
...
def _extract_page_texts(
    ...
    if len(page_texts) != 103 or len(page_texts) != len(expected):
        raise ValueError("document 9 page denominator drift")
    for page_text, page_row in zip(page_texts, expected, strict=True):
        ...
        if (
            ...
            raise ValueError("document 9 page replay drift")
    return page_texts
...
def _load_review(
    ...
    review = _strict_json(REVIEW_PATH, "document 9 source review")
...
def _load_candidates(
    ...
    if (
        ...
        raise ValueError("document 9 candidate raw identity drift")
    value = source_tools.strict_parse_document(raw, "document 9 candidates")
    if not isinstance(value, dict):
        raise ValueError("document 9 candidates are not an object")
    ...
    if (
        ...
        raise ValueError("document 9 candidate content identity drift")
    return value
...
def validate_review(
    ...
    if method != {
        ...
        "whole_page_review": "all_103_pages_including_empty_occurrence_pages",
