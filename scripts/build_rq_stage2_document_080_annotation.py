...
OUTPUT_PATH = ANNOTATION_ROOT / "document_080_q2021_annotation_v1.json"
CAPTURE_ROOT = (
...
def _document_identity(
    ...
    if (
        ...
        raise ValueError("document 80 candidate-index resolution drift")
...
def _extract_page_texts(
    ...
    if questionnaire_inventory._pdftotext_version() != "26.04.0":
        raise ValueError("document 80 Poppler version drift")
    ...
    if len(page_texts) != 531 or len(page_texts) != len(expected):
        raise ValueError("document 80 page denominator drift")
    for page_text, page_row in zip(page_texts, expected, strict=True):
        ...
        if (
            ...
            raise ValueError("document 80 page replay drift")
    return page_texts
...
def _load_candidates(
    ...
    if (
        ...
        raise ValueError("document 80 candidate raw identity drift")
    value = source_tools.strict_parse_document(raw, "document 80 candidates")
    if not isinstance(value, dict):
        raise ValueError("document 80 candidates are not an object")
    ...
    if (
        ...
        raise ValueError("document 80 candidate content identity drift")
    return value
...
def _match_candidate_occurrences(
    ...
    for output in occurrences:
        ...
        outputs_by_coordinate[coordinate].append(output)
    ...
    for candidate in candidate_rows:
        ...
        matches = outputs_by_coordinate.get(coordinate, [])
        exact = bool(matches)
        if not matches:
            matches = [
                output
                for output in outputs_by_page_kind.get(
                    (
                        candidate["page_number"],
                        candidate["occurrence_kind_candidate"],
                    ),
