#!/usr/bin/env python3
"""Build and validate the stage-2 nonauthority annotation for document 11.
...
REVIEW_PATH = (
    ANNOTATION_ROOT / "document_011_fam1973_QxQs_source_review_v1.json"
)
OUTPUT_PATH = ANNOTATION_ROOT / "document_011_fam1973_QxQs_annotation_v1.json"
CAPTURE_ROOT = (
...
def _document_identity(
    ...
    if (
        ...
        raise ValueError("document 11 candidate-index resolution drift")
...
def _extract_page_texts(
    ...
    if questionnaire_inventory._pdftotext_version() != "26.04.0":
        raise ValueError("document 11 Poppler version drift")
    ...
    if len(page_texts) != 68 or len(page_texts) != len(expected):
        raise ValueError("document 11 page denominator drift")
    for page_text, page_row in zip(page_texts, expected, strict=True):
        ...
        if (
            ...
            raise ValueError("document 11 page replay drift")
    return page_texts
...
def _load_review(
    ...
    review = _strict_json(REVIEW_PATH, "document 11 source review")
...
def _load_candidates(
    ...
    if (
        ...
        raise ValueError("document 11 candidate raw identity drift")
    value = source_tools.strict_parse_document(raw, "document 11 candidates")
    if not isinstance(value, dict):
        raise ValueError("document 11 candidates are not an object")
    ...
    if (
        ...
        raise ValueError("document 11 candidate content identity drift")
    return value
...
def _source_printed_identifier(page_text: str, byte_start: int) -> str | None:
    for line in stage1_candidates._physical_lines(page_text):
        ...
        if line_byte_start <= byte_start < line_byte_end:
            ...
            # The 1973 OCR commonly inserts whitespace before the period, so
...
def validate_review(
    ...
    for spec in occurrence_specs:
        ...
        last_order = order
    # The independent document-11 source ledger selects one semantic atom at
        ...
        for position, left in enumerate(same_kind_specs):
            for right in same_kind_specs[position + 1 :]:
                if right["utf8_byte_start"] >= left["utf8_byte_end"]:
                    break
                raise ValueError(
                    "document 11 source review has partially overlapping "
                    "same-kind atoms"
                )
    branch_specs = [
        spec
...
def _match_candidate_occurrences(
    ...
    This relation adjudicates the candidate occurrence atom (page, span, and
    kind) only.  Candidate ancestry is a distinct ``flow_path`` row kind and
    is adjudicated independently below, so an exact atom can be accepted while
    its machine-proposed path is modified or rejected.
    """
...
def _validate_serialized_laws(
    value: Mapping[str, Any], candidates: Mapping[str, Any]
) -> None:
    """Validate row laws before comparing with the rebuilt source projection.

    Running these checks first makes the ancestry, one-to-one label, cycle,
    path, local-only classification, and adjudication mutation tests exercise
    their own rejection laws instead of stopping at a generic array mismatch.
    The later exact-array comparison remains the completeness gate for lawful
    but incomplete subsets such as a selected proper subset of source paths.
    """
...
def validate_annotation(
    ...
    _validate_serialized_laws(value, candidates)
...
def run_mutation_tests(
    ...
    expected_failure_fragments = {
        "unresolved_branch": "flow branch ancestry or identity drift",
        "later_parent": "flow branch ancestry or identity drift",
        "cyclic_branch": "flow branch ancestry or identity drift",
        "omitted_label": "flow label/branch one-to-one cover drift",
        "duplicate_label": "flow branch ancestry or identity drift",
        "selected_path_subset": (
            "document annotation questionnaire_occurrence_rows drift"
        ),
        "inferred_alias": "local repeat inferred an alias relation",
    }
    for name, mutate in _mutation_specs(value):
        ...
        try:
            ...
            expected_fragment = expected_failure_fragments.get(name)
            if expected_fragment is not None and expected_fragment not in str(
                error
            ):
                raise ValueError(
