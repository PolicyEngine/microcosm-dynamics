#!/usr/bin/env python3
"""Build and validate the stage-2 nonauthority annotation for document 62.
...
REVIEW_PATH = (
    ANNOTATION_ROOT / "document_062_fam2001_QxQs_source_review_v1.json"
)
OUTPUT_PATH = ANNOTATION_ROOT / "document_062_fam2001_QxQs_annotation_v1.json"
CAPTURE_ROOT = (
...
def _document_identity(
    ...
    if (
        ...
        raise ValueError("document 62 candidate-index resolution drift")
...
def _extract_page_texts(
    ...
    if questionnaire_inventory._pdftotext_version() != "26.04.0":
        raise ValueError("document 62 Poppler version drift")
    ...
    if len(page_texts) != 41 or len(page_texts) != len(expected):
        raise ValueError("document 62 page denominator drift")
    for page_text, page_row in zip(page_texts, expected, strict=True):
        ...
        if (
            ...
            raise ValueError("document 62 page replay drift")
    return page_texts
...
def _load_review(
    ...
    review = _strict_json(REVIEW_PATH, "document 62 source review")
...
def _load_candidates(
    ...
    if (
        ...
        raise ValueError("document 62 candidate raw identity drift")
    value = source_tools.strict_parse_document(raw, "document 62 candidates")
    if not isinstance(value, dict):
        raise ValueError("document 62 candidates are not an object")
    ...
    if (
        ...
        raise ValueError("document 62 candidate content identity drift")
    return value
...
def validate_review(
    ...
    if method != {
        ...
        "whole_page_review": "all_41_pages_including_empty_occurrence_pages",
...
def _validate_compatibility_predicate() -> None:
    ...
    root_row = {"interview_wave": 2001, "flow_branch_paths": [root]}
    left_or_right = {
        "interview_wave": 2001,
        "flow_branch_paths": [left, right],
    }
    left_row = {"interview_wave": 2001, "flow_branch_paths": [left]}
    right_row = {"interview_wave": 2001, "flow_branch_paths": [right]}
